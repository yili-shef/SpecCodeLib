program alignpccndsstar
  ...
  real(sp) :: afntmp, dpdx, dpdy, dpdz, sstmp
  ...
  if (iargc() .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./align-pc-cndsstar.x nx filelist prefix'
  ...
  open (28, file = 'align-pc-cndsstar-'//trim(flnm)//'.dat')
  write(28,*) 
  ...
  do while ( .not. eof(30) )
    ...
    do ii = 1, nx
      ...
      sstmp = - 3 * sqrt(6.) * alpha * beta * gmma &
          / (alpha*alpha + beta*beta + gmma*gmma)**1.5

      if ( sstmp .ge. .5_sp ) then
          cnt = cnt + 1
  ...
  write(*,*) 'align-pc-cndsstar.x done.'

end program alignpccndsstar
