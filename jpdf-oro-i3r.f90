program jpdforoi3r
  ...
  real(sp), allocatable, dimension(:,:,:) :: g, tmp1, tmp2
  real(sp), allocatable, dimension(:)     :: kx,ky,kz

  integer, parameter :: npnt=160
  real(sp), parameter :: bound = 10., binw=2.*bound/npnt
  real(dp), dimension(npnt,npnt) :: jpdf

  real(sp), dimension(3,3) :: rij
  real(dp) :: mean, rms, skew, rmsr
  real(sp) :: delta_c, ignore_me, tmp, const, mean0, rms0, i3r, rmsr0
  character(80) :: fnm, str, fpath

  write(*,*) 
  write(*,'(''>>>>>> joint PDFs of the Rij*oi*oj and i3r <<<<<<'')')
  write(*,*)
  ll=iargc()
  if (ll .ne. 3) then
          write(*,*)
          ...
          write(*,*) 
          write(*,*) ' Usage: ./jpdf-oro-i3r.x nx filelist ndel'
  ...
  do while ( .not. eof(20) )
    ...
    if ( nfile .eq. 0 ) then
      ...
      wy = cmplx( real(r11) * real(r11) + real(r22) * real(r22) & 
      ...
      rmsr0 = sum( real( wy(1:lx,:,:) ) ) + sum( aimag( wy(1:lx,:,:) ) )
      rmsr0 = sqrt( rmsr0 / (nx*ny*nz) )

      write(*,*) 'Estimated rmsr = ', rmsr0
    end if
    ...
    do ii=1,nx
      ...
      if ( ll .le. npnt .and. ll .ge. 1 .and. mm .le. npnt .and. mm .ge. 1 ) then 
        jpdf(ll,mm) = jpdf(ll,mm) + 1
      end if
  ...
  open(15, file='jpdf-oro-i3r'//fnm(1:len_trim(fnm)))
    do jj = 1, npnt
    do ii = 1, npnt
      write(15,'(15E15.6)') (-bound+(ii-.5)*binw)*rms0/rms, &
      (-bound+(jj-.5)*binw)*rmsr0**3/rmsr**3, jpdf(ii,jj)*rms/rms0*rmsr**3/rmsr0**3
    end do
    end do
  close(15)
