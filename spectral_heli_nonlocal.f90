PROGRAM spectral
  ...
  REAL(SP) :: dt, rnu, time, eps, eta, fkmax, akp, u0, hrel
  LOGICAL  :: new
  ! Parameters should be input from 'parameter_heli_nonlocal.d' initially.
  ...
  OPEN(90,file='parameter_heli_nonlocal.d',status='unknown')
    READ(90,*) forcetype
    READ(90,*) nx
    READ(90,*) ny
    READ(90,*) nz
...
! WRITE out the total energy in K space
  ...
  IF (MOD(jstep,ieout).EQ.0) THEN
    ...
    tmp(1,:,:) = 0.5_SP*tmp(1,:,:)
    hek = SUM(tmp)
    WRITE(*,*)jstep,time,ek,hek,S
    WRITE(25,*)jstep,time,ek,hek,S
  END IF
...
!========================== Forcing term =================================  
  IF (forcetype .NE. 0) CALL force_heli(fx,fy,fz,vx,vy,vz, &
                                            wx,wy,wz,k2,kx,ky,kz,lx1,ly,lz,   &
