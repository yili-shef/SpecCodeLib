program defmsijincij
  ...
  open(27, file = 'defm-alignsc-sgca-pdf-'//trim(flnm)//'.dat')
  open(26, file = 'defm-alignsc-sgca-mean-'//trim(flnm)//'.dat')
  ...
  do while ( .not. eof(30) )
    ...
    do ifile = 1, filecnt
      ...
      do ii = 1, nx
        ...
        evcross(1) = evtrsij(2,3) * evtrcij(3,1) - evtrsij(3,3) * evtrcij(2,1)
        evcross(2) = evtrsij(3,3) * evtrcij(1,1) - evtrsij(1,3) * evtrcij(3,1)
        evcross(3) = evtrsij(1,3) * evtrcij(2,1) - evtrsij(2,3) * evtrcij(1,1)

        scalign = dot_product(evtrsij(:,3), evtrcij(:,1))
