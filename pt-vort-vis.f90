program ptvortvis
  ...
  integer :: nx,ny,nz,ii,jj,kk,lx1,lx,ly,lz,nfile,ndel, nprtcle, ip
  real(sp) :: ignore_me, delta_c, const, dt, time 
  real(sp) :: meanww, meandiffww, meandissww
  ...
  real(sp),    allocatable, dimension(:,:,:) :: ww, diffww, dissww, g, k2
  ...
  if (nx .ne. 6) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./pt-vort-vis.x nx filelist ndel nprtcle dt prefix'
  ...
  open(25, file = 'ww-vis-'//flnm(1:len_trim(flnm))//'.dat')
  ...
  do while ( .not. eof(30) )
    ...
    write(25, '(10E15.4)') time, meanww, meandiffww, meandissww
    time = time + dt
