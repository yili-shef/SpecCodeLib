program pdfevcnd
  ...
  if (ll .ne. 4) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./pdfev-cnd.x nx filelist disslist nthresh'
  ...
  str='-'//fnm(1:len_trim(fnm))//'-cndhd'//disslist(1:len_trim(disslist))//'.dat'
  ...
  open(21, file = disslist( 1 : len_trim( disslist ) ) )
    ...
    do while ( .not. eof(20)) 
      ...
      do ii=1,nx
        ...
        if ( const .ge. nthresh * mdissrate ) then
          numpnt = numpnt + 1
