...
PROGRAM MTLM_uT
    ...
    real(8) d,mag,ratio,wn,t2,ur,u1,u2,v1,v2,tadv
    real(8) spec2d,kx,k22,norm_factor,s2,upl,scalar_spec2d,theta_rmsl
    real(8) Rex,Imx,Rey,Imy,ss,tn,time_scale,total_adv,next_total
    real(8) thetapl
    
    
    real ran2
    
    integer num(nt+1),ind,jx,jy,idum,ky,nadv,ad
    integer kyky,nnr
    character(24) cmd
    character(4) gen
    ...
    s2 = 0.5d0 * scalar_spec2d(1.0d0)
    do jx = 2, lh-2
      s2 = s2 + 0.5d0*(scalar_spec2d(dble(jx-1)) + scalar_spec2d(dble(jx)))
    end do
    Int0kmaxscalar = s2  ! = theta^2/2
    thetarms0 = sqrt(2.d0 * Int0kmaxscalar)
    ! The spectra and urms0 and thetarms0 are to be maintained through the mapping. 

    ! The Ekman frictional coefficient.
    alpha = diss / Int0kmax / 2.

    ! Dissipation for scalar TODO
    ...
    call dfftw_plan_dft_r2c_2d(FTth,nnr,nnr,theta,theta,FFTW_MEASURE)
        ...
        if (k22 < max_k2) then
          ...
          if (ind < nth) then ! nth = half of nt
            ...
            if (Et(ind) < 2.d-38) then
                ...
                ratio = sqrt( scalar_spec2d(wn) )/ Et(ind)
            end if
    ...
    !...second rescale to match the total variance
    s2 = 0.5d0 * Etloc(1)
    do jx = 2,lh-2
      s2 = s2 + 0.5d0*(Etloc(jx-1) + Etloc(jx))
    end do
    upl = sqrt( s2 * 2.d0  )
    resc_factor = thetarms0/upl 
    ...
    call apply_MTLM(vx512,vy512,theta512, nbnd*64 + 2,IFTvx512,IFTvy512,    &
                    IFTth512,FTvx512,FTvy512,FTth512)
    
    call apply_MTLM(vx1024,vy1024,theta1024, nbnd*128 + 2,IFTvx1024,IFTvy1024,    &
                    IFTth1024,FTvx1024,FTvy1024,FTth1024)
    
    call apply_MTLM(vx2048,vy2048,theta2048, nbnd*256 + 2,IFTvx2048,IFTvy2048,    &
                    IFTth2048,FTvx2048,FTvy2048,FTth2048)
    ...
    ! compute urms to estimate the time step size
    ur = 0.0d0
    do jy = 1,ny
    do jx = 1,nxh
       u1 =  real(vx(jx,jy))
    ...
    print *,'For velocity resc= ',resc_factor
    ...
    print *,'For scalar resc= ',resc_factor
    ...
    WRITE(*,"(//,' MTLM2d-scalar-highres HAS FINISHED NORMALLY')")
...
subroutine spectrum(iu,name, output)
    ...
    integer :: jx, jy, ind, kx, ky, kyky, k22
    real(8) :: s2, spec2d, scalar_spec2d, upl, thetapl
    ...
    if (iu > 0) then
      ...
      open(unit = iu, file = name, status = 'unknown')
        ...
        write(iu,"('#      k      E(k) discrete   E(k) analytical Et(k) discrete Et(k) analytical')")

        do jx = 1,nth - 1
          write(iu,"(f11.5,4(es16.6))") real(jx), E(jx), spec2d(dble(jx)), Et(jx), &
          scalar_spec2d(dble(jx)) 
        end do
      
        epsl = 2.d0*alpha*s2 ! Ekman dissipation
        
        ! TODO scalar dissipation
        ...
        write(iu,"('# (Fourier) rms scalar   =',ES13.6)") thetapl
    ...
    if(output) then
      ...
      write(*,"('# (Fourier) rms scalar   =',ES13.6)") thetapl
...
SUBROUTINE MTLM(vxred,vyred,thetared,ldr,tadv,urmsb,thetarmsb)
    ...
    real(8) dt,maxd,wf,hxr,hyr,R,minwf,summat,tadv,urmsb,ur
    real(8) ua1,va1,ua2,va2,ir,jr,dirl2,dirh2,djrl2,djrh2
    integer il,ih,jl,jh,ia,ja,ia1,ia2,i,j,nxr,nyr,i1,i2
    real(8) swfs,Swfu,Swfv,R2,Ra,u1,v1,rfac,thetarmsb
