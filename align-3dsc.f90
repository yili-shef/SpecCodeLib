program align3dsc
  ...
  if (iargc() .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./align-3dsc.x nx file# prefix'
  ...
  do ii = 1, nx
    ...
    cos1 = sum( evtrss(:,3) * evtrcc(:,2) )
    cos2 = sum( evtrss(:,3) * evtrcc(:,1) )
    cosphi = cos1 / sqrt( cos1 * cos1 + cos2 * cos2 )
    phi = acos( abs( cosphi ) )
    cos3 = sum( evtrcc(:,1) * evtrss(:,2) )
  ...
  open(15, file = 'p3d-align3dsc-'//prf(1:len_trim(prf))//flnm(1:len_trim(flnm))//'.dat')
  ...
  write(*,*) 'align-3dsc.x done.'

end program align3dsc
