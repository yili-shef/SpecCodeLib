program otovtk
  ...
  real(sp),    allocatable, dimension(:,:,:) :: g, xinorm, rux, ruy, ruz
  ...
  if (ll .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./vort-xi-vtk.x nx filelist ndel'
  ...
  open(20,file=fnm(1:len_trim(fnm))//'.list')
    do while ( .not. eof(20)) 
      ...
      xinorm = sqrt( rux * rux + ruy * ruy + ruz * ruz )

      ! Change suffix and appending 0 at the end for C output.
      fpath = "xi-omega-"//str(1:len_trim(str)-3)//"vtk0"

      ll = vtkopenfile(fpath( 1:len_trim(fpath) )) 
      ll = vtk_sp_header(sp, nz, ny, nx, 0._sp, 0._sp, 0._sp, 1._sp, 1._sp, 1._sp)

      fpath = "omeganorm0"
      ll = vtkscalar (xinorm, sp, nz, ny, nx, fpath(1 : len_trim(fpath)) )
      fpath = "vorticity0"
      ll = vtk3dvtr (ruz, ruy, rux, sp, nz, ny, nx, fpath(1 : len_trim(fpath)) )


      rux(1:nx:2,:,:) = real(  xix(1:lx,:,:) )
      rux(2:nx:2,:,:) = aimag( xix(1:lx,:,:) )
      ruy(1:nx:2,:,:) = real(  xiy(1:lx,:,:) )
      ruy(2:nx:2,:,:) = aimag( xiy(1:lx,:,:) )
      ruz(1:nx:2,:,:) = real(  xiz(1:lx,:,:) )
      ruz(2:nx:2,:,:) = aimag( xiz(1:lx,:,:) )
      xinorm = sqrt( rux * rux + ruy * ruy + ruz * ruz )
      fpath = "xinorm0"
      ll = vtkscalar (xinorm, sp, nz, ny, nx, fpath(1 : len_trim(fpath)) )
      fpath = "xi0"
