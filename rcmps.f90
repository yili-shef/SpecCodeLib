...
program rcmps
  ...
  if (ll .ne. 5) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./rcmps.x nini tmax ro idum ireal'
  ...
  if ( nini .eq. 0 ) then
    ...
    write(str, '(I20)') nini
    str = adjustl(str)
    str = 'rcmps-aij-'//str(1:len_trim(str)) 
    ...
    write(str, '(I20)') nini
    str = adjustl(str)
    str = 'rcmps-gi-'//str(1:len_trim(str)) 
    str = str(1:len_trim(str))//'-'//strireal(1:len_trim(strireal))//'.data'
    open(10, file = str(1:len_trim(str)), form = 'binary')
      read(10) gall
    close(10)
  ...
  do while ( time .le. tmax + dt)
    ...
    do ii = 1, npnt
      ...
      call drift(aij, gi, omega, driftaij0, driftgi0)
      aij1 = aij + driftaij0 * dt + dwij
      gi1 = gi + driftgi0 * dt + dwgi

      call drift(aij1, gi1, omega, driftaij1, driftgi1)
...
subroutine drift(aij, gi, omega, driftaij, driftgi)
  use mconstant
  use mprmtr
  implicit none
