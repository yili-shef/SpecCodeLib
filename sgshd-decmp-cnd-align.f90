...
!  real(sp), parameter :: mdissrate0 = 0.2375 ! Delta =  8 dx
  ...
  real(dp), dimension(npnt,npnt,npnt) :: jpdf3d, cndhdtal, cndhdtbe, cndhdtga

  real(dp) :: const, mdissrate, mpihtal, mpihtbe, mpihtga
  ...
  real :: cthtalrga, cthtberga, cthtgarga, pihtal, pihtbe, pihtga
  ...
  if (ll .ne. 4) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./sgshd-decmp-cnd-align.x nx filelist ndel normdissrate'
  ...
  jpdf3d = 0.d0; cndhdtal = 0.d0; cndhdtbe = 0.d0; cndhdtga = 0.d0
  nfile=0
  mdissrate = 0._dp
  mpihtal = 0._dp; mpihtbe = 0._dp; mpihtga = 0._dp
  open(20,file=fnm(1:len_trim(fnm)))
    ...
    do while ( .not. eof(20)) 
      ...
      do ii=1,nx
        ...
        pihtal = evaltij(3) * evalrij(3) * cthtalral**2 + &
                 evaltij(3) * evalrij(2) * cthtalrbe**2 + &
                 evaltij(3) * evalrij(1) * cthtalrga**2
        pihtal = 2 * pihtal

        pihtbe = evaltij(2) * evalrij(3) * cthtberal**2 + &
                 evaltij(2) * evalrij(2) * cthtberbe**2 + &
                 evaltij(2) * evalrij(1) * cthtberga**2
        pihtbe = 2 * pihtbe

        pihtga = evaltij(1) * evalrij(3) * cthtgaral**2 + &
                 evaltij(1) * evalrij(2) * cthtgarbe**2 + &
                 evaltij(1) * evalrij(1) * cthtgarga**2
        pihtga = 2 * pihtga

        mpihtal = mpihtal + pihtal
        mpihtbe = mpihtbe + pihtbe
        mpihtga = mpihtga + pihtga

        pih = pih / mdissrate0
        pihtal = pihtal / mdissrate0
        pihtbe = pihtbe / mdissrate0
        pihtga = pihtga / mdissrate0
        ...
        if ( ll .ge. 1 .and. ll .le. npnt .and. &
          ...
          cndhdtal(ll,mm,nn) = cndhdtal(ll,mm,nn) + pihtal
          cndhdtbe(ll,mm,nn) = cndhdtbe(ll,mm,nn) + pihtbe
          cndhdtga(ll,mm,nn) = cndhdtga(ll,mm,nn) + pihtga
  ...
  mpihtal = mpihtal * const / nfile
  mpihtbe = mpihtbe * const / nfile
  mpihtga = mpihtga * const / nfile

  cndhdtal = cndhdtal / (jpdf3d + tiny) 
  cndhdtbe = cndhdtbe / (jpdf3d + tiny)
  cndhdtga = cndhdtga / (jpdf3d + tiny)

  jpdf3d = jpdf3d * const / nfile

  write(*,*) 'check jpdf3d: ', sum(jpdf3d)
  write(*,*) 'check mean dissipation rate: ', mdissrate
  write(*,*) 'check decomp. ral: ', mpihtal, 'rbe: ', mpihtbe, 'rga: ', mpihtga

  jpdf3d = jpdf3d / bwcth / bwphi / bwzeta

  open(15,file='cndsgshd-decmp-al'//str(1:len_trim(str)))
    write(15,'(''zone t = "decmp cnd hd", i='', i6, '',j='', i6, '', k='', i6, '',f=point'')') npnt, npnt, npnt
    do kk = 1, npnt
    do jj = 1, npnt
    do ii = 1, npnt
      write(15,'(15E15.5)') (ii-.5)*bwcth, (jj-.5)*bwphi, (kk-.5)*bwzeta, &
                            cndhdtal(ii,jj,kk), jpdf3d(ii,jj,kk) * cndhdtal(ii,jj,kk)
    end do
    end do
    end do
  close(15)

  open(15,file='cndsgshd-decmp-be'//str(1:len_trim(str)))
    write(15,'(''zone t = "decmp cnd hd", i='', i6, '',j='', i6, '', k='', i6, '',f=point'')') npnt, npnt, npnt
    do kk = 1, npnt
    do jj = 1, npnt
    do ii = 1, npnt
      write(15,'(15E15.5)') (ii-.5)*bwcth, (jj-.5)*bwphi, (kk-.5)*bwzeta, &
                            cndhdtbe(ii,jj,kk), jpdf3d(ii,jj,kk) * cndhdtbe(ii,jj,kk) 
    end do
    end do
    end do
  close(15)

  open(15,file='cndsgshd-decmp-ga'//str(1:len_trim(str)))
    write(15,'(''zone t = "decmp cnd hd", i='', i6, '',j='', i6, '', k='', i6, '',f=point'')') npnt, npnt, npnt
    do kk = 1, npnt
    do jj = 1, npnt
    do ii = 1, npnt
      write(15,'(15E15.5)') (ii-.5)*bwcth, (jj-.5)*bwphi, (kk-.5)*bwzeta, &
                            cndhdtga(ii,jj,kk), jpdf3d(ii,jj,kk) * cndhdtga(ii,jj,kk)
    end do
    end do
    end do
  close(15)
