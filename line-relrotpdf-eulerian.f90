...
program linealignpdfeulerian
  ...
  if (iargc() .ne. 5) then
    ...
    write(*,*) 
    write(*,*) ' Usage: ./line-relrotpdf-eulerian.x nx filelist prefix noffset forward'
  ...
  write(*,*) 'Running line-relrotpdf-eulerian.x '//trim(str)//' '//trim(flnm)&
  ...
  open(25, file = 'line-relrotmean-eulerian-'//trim(flnm))
  open(26, file = 'line-relrotpdf-eulerian-'//trim(flnm))

    do mm = 1, npnt

      prelrot(:,mm) = prelrot(:,mm) / (allpnt(mm) + mytiny) 
      write(*,*) 'line-relrotpdf-eulerian.x: prelrot normalization: ', sum(prelrot(:,mm))
  ...
  write(*,*) 'line-relrotpdf-eulerian.x done.'
...
contains
  ...
  subroutine findRotOnPnt(ipntx, ipnty, ipntz, rotInLabLocal, nvec)
    ...
    nvec = nvec / sqrt(dot_product(nvec,nvec)) ! direction of the line element
...
end program linealignpdfeulerian
