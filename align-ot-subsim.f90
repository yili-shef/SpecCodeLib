program alignot
  ...
  if (ll .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./align-ot-subsim.x nx nfile ndel'
  ...
  call rsgstauij(tmp,tmp,wx,g,nx,ny,nz) 
  t11 = t11 - wx
  call rsgstauij( wz, wz,wx,g,nx,ny,nz)
  t22 = t22 - wx
  call rsgstauij( wy, wy,wx,g,nx,ny,nz)
  t33 = t33 - wx
  call rsgstauij(tmp, wz,wx,g,nx,ny,nz)
  t12 = t12 - wx
  call rsgstauij(tmp, wy,wx,g,nx,ny,nz)
  t13 = t13 - wx
  call rsgstauij( wz, wy,wx,g,nx,ny,nz)
  t23 = t23 - wx
  ...
  do ii = 1, lx1
    ...
    wy(ii,jj,kk)=eye*(kz(kk)*tmp(ii,jj,kk)-kx(ii)* wy(ii,jj,kk))
    wz(ii,jj,kk)=eye*(kx(ii)* wz(ii,jj,kk)-ky(jj)*tmp(ii,jj,kk))
  end do
  end do
  end do
  ...
  open(15, file='potalign-subsim-'//fnm(1:len_trim(fnm))//'.dat')
    do ii=1,npnt
