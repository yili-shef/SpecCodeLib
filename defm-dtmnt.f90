program defmjpdfsizeshape
  ...
  real(sp), parameter :: lbnd = 0.9_sp, ubnd = 1.1_sp, binw = (ubnd - lbnd)/npnt

  real(dp), dimension(npnt) :: pdfdet
  ...
  if (iargc() .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./defm-dtmnt.x nx filelist prefix'
  ...
  open(27, file = 'defm-pdf-dtmnt-'//trim(flnm)//'.dat')
  write(27, '("# determinant pdfdet   ")') 
  write(27, *)
  ...
  do while ( .not. eof(30) )
    ...
    do ii = 1, nx
      ...
      ll = floor( (detbij-lbnd) / binw ) + 1
      if (ll .ge. 1 .and. ll .le. npnt) then
          pdfdet(ll) = pdfdet(ll) + 1
      end if
    ...
    pdfdet  = pdfdet  * const
    write(*,*) 'normalization of pdfdet: ', sum(pdfdet)
    pdfdet = pdfdet / binw

    write(27, '( "# index = ", I6)') nfile
    do ll = 1, npnt
      write(27, '(15E15.3)') lbnd + (ll-0.5_sp)*binw, pdfdet(ll)
    end do
    write(27, *) 
    write(27, *) 
