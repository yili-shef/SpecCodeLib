...
subroutine drift(aij, gij, driftaij, driftgij)
  ...
  ! using the deformation tensor as damping term
  ! can also use exp(gam*aij)
  driftgij = - matmul(gij, aij) + (trcinv/3) * (gij - matmul(gij, matmul(a2ij,gij) ) ) 
