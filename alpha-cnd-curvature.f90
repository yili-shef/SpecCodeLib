program alphacndc
  ...
  real(sp), parameter :: bndcurv = 12
  real(sp), parameter :: binwcurv = bndcurv/npnt

  real(dp), dimension(npnt) :: pdfcurv, alfcndc
  ...
  if (ll .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./alpha-cnd-curvature.x nx filelist ndel'
  ...
  alfcndc = alfcndc / (pdfcurv + mytiny)
  ...
  open(15,file='alpha-cnd-curvature'//str(1:len_trim(str)))
