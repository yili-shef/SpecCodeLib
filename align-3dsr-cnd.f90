program align3dsrcnd
  ...
  real :: ctheta, cos1, cos2, cosphi, phi, cos3, cos4, coszeta, zeta, disstmp
  ...
  if (ll .ne. 5) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./align-3dsr-cnd.x nx filelist disslist ndel nthresh'
  ...
  str='-'//str(1:len_trim(str))//'dx-sr-cnd'//disslist(1:len_trim(disslist))//'-'//fnm(1:len_trim(fnm))//'.dat'
    ...
    do while ( .not. eof(20)) 
      ...
      do ii=1,nx
        ...
        if ( mod(ii,2) .eq. 1 ) then
          ll = (ii+1)/2
          disstmp = real( dissrate(ll,jj,kk) )
        else
          ll = ii/2
          disstmp = aimag( dissrate(ll,jj,kk) )
        end if

        if ( disstmp .ge. nthresh * mdissrate ) then
