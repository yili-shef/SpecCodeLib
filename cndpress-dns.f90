program condpress
  ...
  real(dp) :: sijsij, sijsij0, dtmp, q, r, pq, pr
  ...
  if (ll .ne. 4) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./cndpress-dns.x nx dnsfilelist ndel filtertype '
          write(*,*) '        nx: resolution'
  ...
  ! output
  str='p-hessian-dns-'//strndel(1:len_trim(strndel))//'dx-'//fnm(1:len_trim(fnm))//'.dat'
  open(15, file = str( 1:len_trim(str) ) )
    ...
    do ii = 1, npdf
    do jj = 1, npdf
      write(15,'(8e18.5)') (-bnd + (jj-.5)*binw ) * dtmp,  (-bnd + (ii-.5)*binw ) * dtmp**1.5, &
                           cndpr(ii,jj) * dtmp**4 * pdfqr(ii,jj) / dtmp**2.5, &
                           cndpq(ii,jj) * dtmp**3 * pdfqr(ii,jj) / dtmp**2.5, &
                           pdfqr(ii,jj) / dtmp**2
    end do
    end do
  close(15) 
