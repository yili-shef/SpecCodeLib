program jpdfrstarpih
  ...
  real, parameter :: rbound = 1., pbound = 45.
  ...
  real :: const, meanhd0, rstar
  real :: delta_c, ignore_me, pih
  character(80) :: fnm, str, str1, fpath

  write(*,*) 
  write(*,'(''>>> Joint PDFs of between the r star of rij and sgs helicity dissipation <<<'')')
  write(*,*)
  ll=iargc()
  if (ll .ne. 5) then
          write(*,*)
          ...
          write(*,*) 
          write(*,*) ' Usage: ./jpdf-rstar-pih.x nx filelist ndel nfilter normdissrate'
  ...
  open(20,file=fnm(1:len_trim(fnm)))
    ...
    do while ( .not. eof(20)) 
      ...
      do ii=1,nx
        ...
        rstar = -3*sqrt(6.)*evalues(1)*evalues(2)*evalues(3)
        rstar = rstar/( sqrt( sum( evalues*evalues ) ) )**3
  ...
  open(15, file = 'jpdf-rstar-pih'//str( 1:len_trim(str) ) )
    write(15,*) 'Zone T= "(Pi_H, rstar)", i=', npntp, ', j=', npntr, ', F=point'
    do jj=1,npntr
    do ii=1,npntp
      write(15,*) ( -pbound + (ii-.5) * pbinw ), &
                  ( -rbound + (jj-.5) * rbinw ), jpdf(ii,jj)
    end do
    end do
  close(15)
