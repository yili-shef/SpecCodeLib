...
! Calculate the total KTE and urms
...
do jx = 1,nth
   ...
   k22 = kx*kx + kyky + kzkz
   if (k22 < 1.d-20) cycle
   if(k22 < max_k2) then      !!! Each time the wavenumber is calculated on the fly.
       wn = sqrt(k22)
       ind = int(wn + 0.5d0)
       if (ind < 1) ind = 1   !!! safeguarding 
       ...
       d = sqrt(2.d0*spec3d(wn)/dble(num(ind)))  !!! prescribed spectrum
...
do jx = 1, nth
   ...
   if (k22 < max_k2) then
     ...
     if (ind < nth) then
       if (E(ind) < 2.d-38) then !!!!????? Shouldn't be Et(ind)????
          ratio = 0.0d0
       else  
...
! The urb is taken as a portion of the total rms. The portion is calculated the energy ratio.
...
do ka = 1,nzr
if((ka/4)*4 == ka) write(*,"('*',$)")  !!! what for??? check ka is the multiple of 4 ??
!!!! I see: to control output, print *s every four ka's
do ja = 1,nyr
do ia = 1,nxr/2
   ia2 = 2*ia
   ia1 = ia2 - 1
