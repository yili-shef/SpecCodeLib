program alignis
  ...
  if (iargc() .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./align-is.x nx filelist prefix'
  ...
  open(27, file = 'align-is2d-'//trim(flnm)//'.dat')
  open(26, file = 'align-is1d-'//trim(flnm)//'.dat')
  ...
  do while ( .not. eof(30) )
    ...
    do ii = 1, nx
      ...
      ! strain rate tensor
      aa = 0.5_sp * ( aa + transpose(aa) )

      ! The interface of subroutine dsyevr
      ! call dsyevr(jobz, range, uplo, n, a, lda, vl, vu, il, iu, abstol, m, w, z, ldz, isuppz, work,
      ! lwork, iwork, liwork, info)

      ! aa is the strain rate tensor sij. 
      ! evcc contains the eigenvalues of sij.
      ! evtrcc contains the eigenvectors of sij.
      call dsyevr("V", "A", "U", evsize, aa, evsize, iignore, iignore, iignore, iignore, dlamch('S'), &
