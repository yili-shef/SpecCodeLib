program eulerdns
  ...
  real(sp) :: timemax, beta, dtw, time, akp, u0
  logical  :: new
  ...
  open(90,file='parameter_euler.d',status='unknown')
    read(90,*) nx
    read(90,*) ny
    read(90,*) nz
    read(90,*) timemax
  ...
  do while ( abs(time) .le. abs(timemax) )
    ...
    if ( beta .ge. 0 ) then
        ...
        dt=max(betadx/umax,3._sp*dto)
        if (dt .le. twrite-time) then
                dt=twrite-time
        else if (twrite-time-dt .gt. .2_sp*dt) then
                dt=.5_sp*(twrite-time)
        end if
    ...
    case (0)
      ...
      time = time + dt
      dto=dt
      call output(wx,wy,wz,99999,lx1,ly,lz)
    ...
    case (1)
      ...
      vx = S*ox + ek*vx + delta*wx 
      vy = S*oy + ek*vy + delta*wy 
      vz = S*oz + ek*vz + delta*wz 
 
      time = time + dt
      dto=dt+dto
      call input(ox,oy,oz,99999,lx1,ly,lz)
