program spectral_dns
  ...
  integer  :: lx, ly, lz, lx1, OMP_get_thread_num
  ...
  do while (time .le. timemax)
    ...
    elsewhere
      ...
      !!$omp critical (tracker1)
      !write(*,*) 'Thread', TID, 'in k2_e'
      !!$omp end critical (tracker1)
    !!$omp end workshare
    !!$omp end parallel
    ...
    where ( k2 .lt. fkmax2 )
        ...
        wz = wz + coeforce * vz
    endwhere
      !!$omp critical (tracker2)
      !write(*,*) 'Thread', TID, 'in force'
      !!$omp end critical (tracker2)
    !!$omp end workshare
    !!$omp end parallel
    ...
    !$omp parallel num_threads(numth)
    !$omp workshare
    k2_e = (kx*real(wy) + ky*real(wz) + kz*real(wx))/k2
    wy = cmplx(real(wy) - kx*k2_e, aimag(wy))
    wz = cmplx(real(wz) - ky*k2_e, aimag(wz))
    wx = cmplx(real(wx) - kz*k2_e, aimag(wx))
    k2_e = (kx*aimag(wy) + ky*aimag(wz) + kz*aimag(wx))/k2
    wy = cmplx(real(wy), aimag(wy) - kx*k2_e)
    wz = cmplx(real(wz), aimag(wz) - ky*k2_e)
    wx = cmplx(real(wx), aimag(wx) - kz*k2_e)
    !$omp end workshare
    !$omp end parallel
