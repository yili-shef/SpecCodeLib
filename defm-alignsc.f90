program defmsijincij
  ...
  if (iargc() .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./defm-alignsc.x nx filelist prefix'
  ...
  open(27, file = 'defm-alignsc-pdf-'//trim(flnm)//'.dat')
  open(26, file = 'defm-alignsc-mean-'//trim(flnm)//'.dat')
  ...
  do while ( .not. eof(30) )
    ...
    do ifile = 1, filecnt
      ...
      do ii = 1, nx
        ...
        ! Find the different contributions to rotation rate of the S-frame
        ! Need to find the mean, as well as the average conditioned on alignment

        ! Find the contributions to the C-frame
        ...
        ! evtrsij(:,1) cross evtrcij(:,1)
        evcross(1) = evtrsij(2,1) * evtrcij(3,1) - evtrsij(3,1) * evtrcij(2,1)
        evcross(2) = evtrsij(3,1) * evtrcij(1,1) - evtrsij(1,1) * evtrcij(3,1)
        evcross(3) = evtrsij(1,1) * evtrcij(2,1) - evtrsij(2,1) * evtrcij(1,1)

        scalign = dot_product(evtrsij(:,1), evtrcij(:,1))
