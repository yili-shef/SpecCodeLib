program jpdfoc
  ...
  real(dp), dimension(npnto, npntx) :: joc

  real(dp) :: rmso0, rmsx0, rmso, rmsx


  write(*,*) 
  write(*,'(''>>>>>> JPDF of omega and curvature <<<<<<'')')
  write(*,*)
  ll=iargc()
  if (ll .ne. 3) then
          write(*,*)
          ...
          write(*,*) 
          write(*,*) ' Usage: ./jpdf-oc-s(d)p.x nx filelist ndel'
  ...
  joc = joc / (nfile * nx * ny * nz)
  write(*,*) 'Check normalization of joc: ', sum(joc)
  joc = joc / bwo / bwx

  open(15, file ='jpdfoc-'//str(1:len_trim(str))//'dx-'//fnm(1:len_trim(fnm))//'.dat' )
    write(15,*) '# Zone T= "(omega,curvature)", i=', npnto, ', j=', npntx, ', F=point'
    do ii = 1, npnto
    do jj = 1, npntx
      write(15,*) (ii-.5)*bwo*rmso0/rmso, (jj-.5)*bwx*rmsx0/rmso0*rmso/rmsx, &
                  joc(ii,jj)*rmsx/rmsx0
    end do
    write(15,*)
    end do
  close(15)
