...
module mprmtr
  ...
  ! controls
  real(sp), parameter :: dt = 1.e-4, dtwrite = 0.1 
...
program velgradmod
  ...
  real(sp), dimension(three, three, three, three) :: v, upaijmn, umaijmn, upgijmn, umgijmn
  real(sp), dimension(three, three, three, three) :: rpaijmn, rmaijmn, rpgijmn, rmgijmn
  ...
  if ( nini .eq. 0 ) then
    ...
    do ll = 1, npnt
      ...
      call taylorch(gij, tmp2, three)
  
      gij = matmul(transpose(gij), gij)
      tmp2 = gij(1,1) + gij(2,2) + gij(3,3) ! Tr(C^{-1})
...
subroutine drift(aij, gij, driftgij)
  ...
  driftgij = tra2 * aij / 3 &
             ...
             - trcinv * gij / 3 + 2 * gmma * trcinvg * aij / 3  &
