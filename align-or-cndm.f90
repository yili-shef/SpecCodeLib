program alignor
  ...
  if (ll .ne. 5) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./align-or-cndm.x nx filelist ndel disslist nthresh'
  ...
  open(21, file = disslist( 1 : len_trim( disslist ) )//'.list' )
    ...
    do while ( .not. eof(20) )
      ...
      do ii=1,nx
        ...
        if ( ignore_me .le. -nthresh * mdissrate ) then
  ...
  fnm = '-'//disslist(1:len_trim(disslist))//'-cndm'//thresh(1:len_trim(thresh))//'-'//fnm(1:len_trim(fnm))//'.dat'
