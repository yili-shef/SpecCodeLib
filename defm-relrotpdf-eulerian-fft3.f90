...
program defmalignpdfeulerian
  ...
  if (iargc() .ne. 5) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./defm-relrotpdf-eulerian.x nx filelist prefix noffset forward'
  ...
  open(25, file = 'defm-relrotmean-eulerian-fft3-'//trim(flnm))
  open(26, file = 'defm-relrotpdf-eulerian-fft3-'//trim(flnm))

    do mm = 1, npnt

      prelrot(:,mm) = prelrot(:,mm) / (allpnt(mm) + mytiny) 
      write(*,*) 'defm-relrotpdf-eulerian-fft3.x: prelrot normalization: ', sum(prelrot(:,mm))
  ...
  write(*,*) 'defm-relrotpdf-eulerian.x done.'
