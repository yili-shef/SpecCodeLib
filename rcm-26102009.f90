...
program rotationcm
  ...
  real(sp) :: ro, omega, twrite, tmax, time, bbb, nbv
  real(sp) :: gasdev
  ...
  if (ll .ne. 5) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./rcm-ddmmyyyy.x nini tmax ro bbb idum ireal'
  ...
  omega = 1./ro
  nbv = 2.*omega * bbb ! Brunt-Vaisala frequency.
  ...
  if ( nini .eq. 0 ) then
    ...
    write(str, '(I20)') nini
    str = adjustl(str)
    str = 'cm-aij-rs-'//str(1:len_trim(str)) 
    ...
    write(str, '(I20)') nini
    str = adjustl(str)
    str = 'cm-gi-rs-'//str(1:len_trim(str)) 
    str = str(1:len_trim(str))//'-'//strireal(1:len_trim(strireal))//'.data'
    open(10, file = str(1:len_trim(str)), form = 'binary')
      read(10) aall
    close(10)
  ...
  do while ( time .le. tmax + dt)
    ...
    do ii = 1, npnt
      ...
      call drift(aij, gi, omega, nbv, driftaij0, driftgi0)
      aij1 = aij + driftaij0 * dt + dwij
      gi1 = gi + driftgi0 * dt + dwgi

      call drift(aij1, gi1, omega, nbv, driftaij1, driftgi1)
...
subroutine drift(aij, gi, omega, nbv, driftaij, driftgi)
  use mconstant
  use mprmtr
  implicit none
