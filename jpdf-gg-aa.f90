program jpggaa
  ...
  real(sp), parameter :: bndgg = 8., bndaa = 4.
  real(sp), parameter :: binwgg = bndgg/npnt, binwaa = bndaa/npnt

  real(dp), dimension(npnt, npnt) :: jpdfggaa

  real(dp) :: const, rmsgg, rmsaa, covgijaij
  real(sp) :: delta_c, ignore_me, rmsgg0, rmsaa0
  ...
  if (ll .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./jpdf-gg-aa.x nx filelist ndel'
  ...
  do while ( .not. eof(20) )
    ...
    do ii = 1, nx
      ...
      covgijaij = covgijaij + sum( gg * aa )

      ignore_me = sum( gg * gg )
      rmsgg = rmsgg + ignore_me

      delta_c = sum( aa * aa )
      rmsaa = rmsaa + delta_c

      ignore_me = ignore_me / rmsgg0**2
      delta_c = delta_c / rmsaa0**2

      ll = floor( ignore_me / binwgg ) + 1
      mm = floor( delta_c  / binwaa ) + 1

      if ( ll .ge. 1 .and. ll .le. npnt .and. mm .ge. 1 .and. mm .le. npnt) then
        jpdfggaa(mm,ll) = jpdfggaa(mm,ll) + 1
      end if
  ...
  covgijaij = covgijaij / nfile * const

  write(*,*) '*******************************'
  write(*,*) 'cov(gij, aij) = ', covgijaij
  write(*,*) '*******************************'
  ...
  jpdfggaa = jpdfggaa / nfile * const
  write(*,*) 'Check normalizationof jpdfggaa:', sum(jpdfggaa)

  jpdfggaa = jpdfggaa / binwgg / binwaa

  open(15,file='jpdf-gg-aa'//str(1:len_trim(str)))
    write(15,*) '# Zone T= "(r,q)", i=', npnt, ', j=', npnt, ', F=point'
    do ii=1,npnt
    do jj=1,npnt
      write(15,'(15E15.5)') ((ii-.5)*binwaa)*rmsaa0**2/rmsaa**2, & 
            ((jj-.5)*binwgg)*rmsgg0**2/rmsgg**2, &
            jpdfggaa(ii,jj)*rmsaa**2*rmsgg*2/rmsaa0**2/rmsgg0**2
    end do
    write(15,*)
    end do
  close(15)
