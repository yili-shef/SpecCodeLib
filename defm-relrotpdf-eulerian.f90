program defmalignpdfeulerian
  ...
  open(25, file = 'defm-relrotmean-eulerian-'//trim(flnm))
  open(26, file = 'defm-relrotpdf-eulerian-'//trim(flnm))

    do mm = 1, npnt

      prelrot(:,mm) = prelrot(:,mm) / (allpnt(mm) + mytiny) 
      write(*,*) 'defm-relrotpdf-eulerian.x: prelrot normalization: ', sum(prelrot(:,mm))
