program curvaturecndrr
  ...
  real(dp), dimension(npnt) :: pdfrr, cndoh
  ...
  write(*,*) 
  write(*,'(''>>>>>> Curvature of vortex lines conditioned rijrij <<<<<<'')')
  write(*,*)
  ll=iargc()
  if (ll .ne. 3) then
          write(*,*)
          ...
          write(*,*) 
          write(*,*) ' Usage: ./curvature-cnd-rr.x nx filelist ndel'
  ...
  do while ( .not. eof(20) )
    ...
    do ii = 1, lx1
      ! rij
      g12(ii,jj,kk) = eye * (kx(ii) * g22(ii,jj,kk) + ky(jj) * g11(ii,jj,kk) ) / 2.
      g13(ii,jj,kk) = eye * (kz(kk) * g11(ii,jj,kk) + kx(ii) * g33(ii,jj,kk) ) / 2.
      g23(ii,jj,kk) = eye * (ky(jj) * g33(ii,jj,kk) + kz(kk) * g22(ii,jj,kk) ) / 2.
    ...
    do ii = 1, nx
      if ( mod(ii,2) .eq. 1 ) then
        ...
        gg(2,1) = real( g12(ll,jj,kk) )
        gg(2,2) = real( g22(ll,jj,kk) )
        gg(2,3) = real( g23(ll,jj,kk) )
        gg(3,1) = real( g13(ll,jj,kk) )
        gg(3,2) = real( g23(ll,jj,kk) )
        ...
        gg(2,1) = aimag( g12(ll,jj,kk) )
        gg(2,2) = aimag( g22(ll,jj,kk) )
        gg(2,3) = aimag( g23(ll,jj,kk) )
        gg(3,1) = aimag( g13(ll,jj,kk) )
        gg(3,2) = aimag( g23(ll,jj,kk) )
  ...
  cndoh = cndoh / (pdfrr + mytiny_dp)

  pdfrr = pdfrr / nfile * const
  write(*,*) 'Check normalization of pdfrr:', sum(pdfrr)

  pdfrr = pdfrr / binwq 


  ! Ouput format for gnuplot
  open(15,file='curvature-cnd-rr'//str(1:len_trim(str)))
    write(15,*) '# Zone T= "curvature | rr", i=', npnt, ', F=point'
    do jj=1,npnt
      write(15,'(15E15.5)') ((jj-.5)*binwq)*rmsrr0**2/rmsrr**2, &
                pdfrr(jj)*rmsrr**2/rmsrr0**2, cndoh(jj)
    end do
  close(15)
