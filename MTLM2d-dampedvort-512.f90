...
SUBROUTINE MTLM(vxred,vyred,thetared,ldr,tadv,urmsb,thetarmsb)
    ...
    do ia = 1,nxr/2
       ...
       theta1 = theta1 * exp(-alpha * tadv * hxr)
    
       ua2 = aimag(vxred(ia,ja))
       va2 = aimag(vyred(ia,ja))
       theta2 = aimag( thetared(ia,ja) )
       theta2 = theta2 * exp(-alpha * tadv * hxr)
    
    !****(1) 
       ! ir = dble(ia1) + dt*ua1  !!! dt has been normalized by dx
       ! jr = dble(ja) + dt*va1

       ir = dble(ia1) + ua1 * ( 1 - exp(-alpha * tadv * hxr) ) / alpha
       jr = dble(ja ) + va1 * ( 1 - exp(-alpha * tadv * hxr) ) / alpha
    ...
    !****(2) 
       ...
       ir = dble(ia2) + ua2 * ( 1 - exp(-alpha * tadv * hxr) ) / alpha
       jr = dble(ja ) + va2 * ( 1 - exp(-alpha * tadv * hxr) ) / alpha
      ...
      do i = 1,nxr
        ...
        do jsearch = js(j),je(j)
          ...
          do isearch = is(i),ie(i)
            ...
            ip = dble(in) + ua1 * ( 1 - exp(-alpha * tadv * hxr) ) / alpha
            jp = dble(jn) + va1 * ( 1 - exp(-alpha * tadv * hxr) ) / alpha
