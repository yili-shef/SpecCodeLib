program msgshdcndss
  ...
  real(dp), dimension(npnts) :: pdfs, mpihcnds
  real(dp) :: rmss
  ...
  write(*,*) 
  write(*,'(''>>>>>> SGS helicity dissipation conditioned on strain rate <<<<<<'')')
  write(*,*) 
  ...
  if (ii .ne. 4) then
          ...
          write(*,*)
          write(*,*) ' Usage: ./meansgshd-cnd-ss.x nx datalist ndel normhelidiss'
  ...
  open(15, file ='msgshd-cndss'//str(1:len_trim(str)) )
    do ii = 1, npnts
      write(15,'(20E18.5)') ((ii-.5)*bws)*rmss0/rmss, pdfs(ii), mpihcnds(ii), pdfs(ii)*mpihcnds(ii)
    end do
  close(15)
