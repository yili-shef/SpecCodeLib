program jpdfpihtstar
  ...
  real(dp),    allocatable, dimension(:,:,:) :: tstar
  real(sp),    allocatable, dimension(:,:,:) :: g
  real(sp),    allocatable, dimension(:)     :: kx, ky, kz

  integer, parameter :: npnts=100, npnth = 900
  real(sp), parameter :: bounds = 1, boundh = 45.
  real(sp), parameter :: bws = 2.*bounds/npnts, bwh = 2.*boundh/npnth
  ...
  real(dp), dimension(npnts, npnth) :: jpihtstar
  ...
  if (ii .ne. 4) then
          ...
          write(*,*)
          write(*,*) ' Usage: ./jpdf-pih-tstar.x nx datalist ndel normhelidiss'
  ...
  allocate(g(lx1,ly,lz),tstar(nx,ny,nz),pih(lx1,ly,lz))
  ...
  open(20,file=dnslist(1:len_trim(dnslist)))
    ...
    do while ( .not. eof(20) )
      ...
      do ii = 1, nx
        ...
        tstar(ii,jj,kk) = - 3 * sqrt(6.) * alpha * beta * gamm &
      ...
      do ii = 1, nx
        ...
        if ( ll .ge. 1 .and. ll .le. npnts .and. mm .ge. 1 .and. &
             mm .le. npnth ) then
          jpihtstar(ll,mm) = jpihtstar(ll,mm) + 1
        end if
  ...
  jpihtstar = jpihtstar / nfile * const

  write(*,*) 'Check normalization of jpihtstar: ', sum(jpihtstar) 

  jpihtstar = jpihtstar / bws / bwh

  open(15, file ='jpihtstar'//str(1:len_trim(str)) )
    write(15,*) 'Zone T= "(tstar,pih)", i=', npnts, ', j=', npnth, ', F=point'
    do jj = 1, npnth
    do ii = 1, npnts
      write(15,*) -bounds+(ii-.5)*bws, -boundh+(jj-.5)*bwh, jpihtstar(ii,jj)
    end do
    end do
  close(15)

  deallocate(kx,ky,kz,ux,uy,uz,g,tstar,pih)
