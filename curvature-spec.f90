program curvaturespec
  ...
  real(dp) :: const
  real(sp) :: meancurv, rmscurv, kxii, kyjj, kzkk
  ...
  do while ( .not. eof(20) )
    ...
    do ii = 1, lx
      ...
      meancurv = sqrt( delta_c ** 2 + ignore_me ** 2 + tmp ** 2)
      ...
      rmscurv = sqrt( delta_c ** 2 + ignore_me ** 2 + tmp ** 2)

      g11(ii,jj,kk) = cmplx( meancurv, rmscurv )
  ...
  open(24,file='spec-curvature'//str(1:len_trim(str)))
  do ii=1,lx
  ...
  deallocate(xix, xiy, xiz, ekcurv)

  call destroyplan3d

  write(*,*) 'Finished'
end program curvaturespec
