program align3drtcndpoo
  ...
  real :: delta_c, ignore_me, const, rmso0
  ...
  if (ll .ne. 4) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./align-3drt-cndpoo.x nx filelist ndel nthresh'
          write(*,*) '        nx: resolution of data'
          write(*,*) '        filelist: the list of the data files, *.list'
          write(*,*) '        ndel: filter scale Delta=ndel*dx'
          write(*,*) '        nthresh: defining the threshold value for oo'
          write(*,*)
          write(*,*) ' Stopped'
          stop
  end if
  ...
  str='-'//str(1:len_trim(str))//'dx-rt-cndpoo'//fpath(1:len_trim(fpath))//'-'//fnm(1:len_trim(fnm))//'.dat'
  ...
  open(20, file = fnm(1:len_trim(fnm)) )
    ...
    do while ( .not. eof(20) ) 
      ...
      if ( numpnt .eq. 0 ) then
        rmso0 = sum( real( wx(1:lx,:,:) ) ) + sum( aimag( wx(1:lx,:,:) ) )
        rmso0 = sqrt(rmso0 * const)

        write(*,*) 'Estimated mean oo: ', rmso0
      end if
      rmso = rmso + sum( real( wx(1:lx,:,:) ) ) + sum( aimag( wx(1:lx,:,:) ) )
      nfile = nfile + 1
      ...
      do ii=1,nx
        ...
        if ( mod(ii,2) .eq. 1 ) then 
          ll = (ii + 1)/2
          ignore_me = sqrt( real( wx(ll,jj,kk) ) )
        else
          ll = ii / 2
          ignore_me = sqrt( aimag( wx(ll,jj,kk) ) )
        end if

        if ( ignore_me .ge. nthresh * rmso0 ) then
