program sgsedmoments
  ...
  real(sp) :: ignore_me, const, delta_c, kc2

  character(80) :: fnm,str,str1,fpath, strfltr

  write(*,*) 
  write(*,'(''>>> Moments of SGS energy dissipation<<<'')')
  write(*,*)
  if (iargc() .ne. 4) then
          write(*,*)
          write(*,*) ' >>>>>> Wrong number of arguments <<<<<<'
          write(*,*) 
          write(*,*) ' Usage: ./sgsedmoments.x nx filelist ndel ifilter'
  ...
  allocate( g(lx1,ly,lz), k2(lx1,ly,lz), enerdiss(lx1,ly,lz) )
  ...
  open( 20, file = fnm( 1:len_trim(fnm) )//'.list' )
    do while ( .not. eof(20) )
      ...
      ! redundant storage to avoid some FFTW
      wx = ux * g; wy = uy * g; wz = uz * g
      ...
      m1  = m1  + sum( ( abs( real(delta_c * enerdiss(1:lx,:,:))) ) **(1./3) ) + &
                  sum( ( abs(aimag(delta_c * enerdiss(1:lx,:,:))) ) **(1./3) )
      m2  = m2  + sum( ( abs( real(delta_c * enerdiss(1:lx,:,:))) ) **(2./3) ) + &
                  sum( ( abs(aimag(delta_c * enerdiss(1:lx,:,:))) ) **(2./3) )
      m3  = m3  + sum( ( abs( real(delta_c * enerdiss(1:lx,:,:))) )          ) + &
                  sum( ( abs(aimag(delta_c * enerdiss(1:lx,:,:))) )          )
      m4  = m4  + sum( ( abs( real(delta_c * enerdiss(1:lx,:,:))) ) **(4./3) ) + &
                  sum( ( abs(aimag(delta_c * enerdiss(1:lx,:,:))) ) **(4./3) )
      m5  = m5  + sum( ( abs( real(delta_c * enerdiss(1:lx,:,:))) ) **(5./3) ) + &
                  sum( ( abs(aimag(delta_c * enerdiss(1:lx,:,:))) ) **(5./3) )
      m6  = m6  + sum( ( abs( real(delta_c * enerdiss(1:lx,:,:))) ) **2      ) + &
                  sum( ( abs(aimag(delta_c * enerdiss(1:lx,:,:))) ) **2      )
      m7  = m7  + sum( ( abs( real(delta_c * enerdiss(1:lx,:,:))) ) **(7./3) ) + &
                  sum( ( abs(aimag(delta_c * enerdiss(1:lx,:,:))) ) **(7./3) )
      m8  = m8  + sum( ( abs( real(delta_c * enerdiss(1:lx,:,:))) ) **(8./3) ) + &
                  sum( ( abs(aimag(delta_c * enerdiss(1:lx,:,:))) ) **(8./3) )
      m9  = m9  + sum( ( abs( real(delta_c * enerdiss(1:lx,:,:))) ) **3 )      + &
                  sum( ( abs(aimag(delta_c * enerdiss(1:lx,:,:))) ) **3 )    
      m10 = m10 + sum( ( abs( real(delta_c * enerdiss(1:lx,:,:))) ) **(10./3)) + & 
                  sum( ( abs(aimag(delta_c * enerdiss(1:lx,:,:))) ) **(10./3))
  ...
  strfltr = 'sgsedmoments-'//strfltr(1:len_trim(strfltr))//'-' 
  ...
  deallocate( kx, ky, kz, k2, g, ux, uy, uz, fux, fuy, fuz, wx, wy, wz, sij, tij, enerdiss )

  call destroyplan3d

  write(*,*) 'sgsedmoments.x finished'

end program sgsedmoments
