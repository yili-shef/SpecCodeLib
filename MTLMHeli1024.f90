...
real(8), parameter :: beta = 5.2d0
real(8), parameter :: c_eta = 0.4

!======== parameter for helicity spectrum ===========
real(8), parameter :: Ch = 1.0d00
real(8) :: diss_heli

!======== added by YL ======
logical, parameter :: fromfile = .false.
...
complex(8), allocatable, dimension(:,:,:) :: vx512,vy512,vz512 
...
integer(8) FTvx512,FTvy512,FTvz512,IFTvx512,IFTvy512,IFTvz512
...
integer num(nt+1),ind,jx,jy,jz,idum,ans,ky,kz,r,rs,nadv,ad
...
!ind = iarg()
!if ( ind .ne. 1 ) stop "need one argument"
    ind = iarg()
    if ( ind .ne. 2 ) stop "need two arguments: ./MTLMHeli512.x -idum nfile"
...
allocate( vx512(nt/4 + 1 ,nt/2 ,nt/2 ),vy512(nt/4 + 1 ,nt/2 ,nt/2 ),vz512(nt/4 + 1 ,nt/2 ,nt/2 )) 
allocate( vx256(nt/8 + 1 ,nt/4 ,nt/4 ),vy256(nt/8 + 1 ,nt/4 ,nt/4 ),vz256(nt/8 + 1 ,nt/4 ,nt/4 )) 
allocate( vx128(nt/16 + 1 ,nt/8 ,nt/8 ),vy128(nt/16 + 1 ,nt/8 ,nt/8 ),vz128(nt/16 + 1 ,nt/8 ,nt/8 ))
allocate( vx64(nt/32 + 1 ,nt/16 ,nt/16),vy64(nt/32 + 1 ,nt/16 ,nt/16 ),vz64(nt/32 + 1 ,nt/16 ,nt/16 ))
allocate( vx32(nt/64 + 1, nt/32,nt/32),vy32(nt/64 + 1, nt/32,nt/32) ,vz32(nt/64 + 1, nt/32,nt/32))
allocate( vx16(nt/128 + 1,nt/64,nt/64),vy16(nt/128 + 1,nt/64,nt/64) ,vz16(nt/128 + 1,nt/64,nt/64))
allocate( vx08(nt/256 + 1,nt/128,nt/128),vy08(nt/256 + 1,nt/128,nt/128),vz08(nt/256 + 1,nt/128,nt/128))
!- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
nxh = nx/2
nyh = ny/2
nzh = nz/2
...
!!!!! parameters are used in spec3d to generate analytic energy spectrum.
eta = Ldom/(nt*phi)
diss = urms*urms*urms/l
rnu = (eta*eta*eta*eta * diss)**(1.d0/3.d0)

!!!! TODO: specify diss_heli !!!!
diss_heli = 3.d0 * diss
...
nnr = nt/2
print *,nnr
call dfftw_plan_dft_c2r_3d(IFTvx512,nnr,nnr,nnr,vx512,vx512,FFTW_MEASURE)
call dfftw_plan_dft_c2r_3d(IFTvy512,nnr,nnr,nnr,vy512,vy512,FFTW_MEASURE)
call dfftw_plan_dft_c2r_3d(IFTvz512,nnr,nnr,nnr,vz512,vz512,FFTW_MEASURE)

call dfftw_plan_dft_r2c_3d(FTvx512,nnr,nnr,nnr,vx512,vx512,FFTW_MEASURE)
call dfftw_plan_dft_r2c_3d(FTvy512,nnr,nnr,nnr,vy512,vy512,FFTW_MEASURE)
call dfftw_plan_dft_r2c_3d(FTvz512,nnr,nnr,nnr,vz512,vz512,FFTW_MEASURE)
...
! along the way the wavenumbers kx ky kz are calculated
num = 0
do jz = 1,nz
   if (jz == nzhp) cycle
   kz = jz - 1
...
!!!! generate a velocity field with fixed energy and helicity spectrum.
call geninit(idum)
...
!TODO: do we need to do this for helicity spectrum as well?
...
call apply_MTLM(vx08,vy08,vz08,nt/128 + 2,                         &
                IFTvx08,IFTvy08,IFTvz08,FTvx08,FTvy08,FTvz08)

call apply_MTLM(vx16,vy16,vz16,nt/64 + 2,                         &
                IFTvx16,IFTvy16,IFTvz16,FTvx16,FTvy16,FTvz16)

call apply_MTLM(vx32,vy32,vz32,nt/32 + 2,                          &
                IFTvx32,IFTvy32,IFTvz32,FTvx32,FTvy32,FTvz32)

call apply_MTLM(vx64,vy64,vz64,nt/16 + 2,                          &
                IFTvx64,IFTvy64,IFTvz64,FTvx64,FTvy64,FTvz64)

call apply_MTLM(vx128,vy128,vz128,nt/8 + 2,IFTvx128,IFTvy128,    &
                IFTvz128,FTvx128,FTvy128,FTvz128)

call apply_MTLM(vx256,vy256,vz256,nt/4 + 2,IFTvx256,IFTvy256,    &
                IFTvz256,FTvx256,FTvy256,FTvz256)

call apply_MTLM(vx512,vy512,vz512,nt/2 + 2,IFTvx512,IFTvy512,    &
                IFTvz512,FTvx512,FTvy512,FTvz512)
...
!       vy(jx,jy,jz) = vy(jx,jy,jz)*ratio
!       vz(jx,jy,jz) = vz(jx,jy,jz)*ratio
!!!!!! Already replaced by call rescale_spec !!!!!!
...
deallocate( vx512,vy512,vz512) 
deallocate( vx256,vy256,vz256) 
deallocate( vx128,vy128,vz128) 
deallocate( vx64,vy64,vz64) 
deallocate( vx32,vy32,vz32) 
deallocate( vx16,vy16,vz16) 
deallocate( vx08,vy08,vz08) 
...
write(*,*) 'did I pass the first loop?'
...
if (iu > 0) then
  ...
  write(*,*) 'Did I pass afterspec data ouput?'
  ...
  write(*,*) 'Did I pass writing to iu?'
close(iu)
end if
  if(output) then
...
!****** 3-D spectrum function ********
  ...
  if ( .not. fromfile ) then
      ...
      spec3d =  Ck * diss**(2.d0/3.d0) * rk**(-5.d0/3.d0)       &
              ...
              * dexp(-beta *((rk*eta)**4.d0+ c_eta**4.d0)**0.25d0 + beta * c_eta)
...
!!!======== helicity spectrum ===============
!!! The expressions in the integral and dissipation scales need some more thoughts.

real(8) function heli_spec3d(rk)
  use global
  implicit none
  ...
  logical, save :: first = .true.
  real(8), save :: helisp(nx/2)
  integer :: jx, ii

  if ( .not. fromfile ) then
      kl = rk * l
      heli_spec3d = Ch * diss_heli * diss**(-1.d0/3.d0) * rk**(-5.d0/3.d0)       &
                * (kl/sqrt(kl*kl + cl))**(5.d0/3.d0 + p0)    &
                * dexp(-beta *((rk*eta)**4.d0+ c_eta**4.d0)**0.25d0 + beta * c_eta)
      !heli_spec3d = Ch * diss_heli * diss**(-1.d0/3.d0) * rk**(-5.d0/3.d0)       
  else
      ...
      if (first .eq. .true.) then
          open(909, file = 'helispec.in')
            do jx = 1, nx/2
              read(909, *) ii, helisp(jx)
            end do
          close(909)
          first = .false.
      end if
      heli_spec3d = helisp( floor(rk+0.5) )

  end if


end function heli_spec3d
...
!!! TODO: this part needs changes for helical turbulence.

write(*, *) 'I am entering rescale_spec'
call rescale_spec
...
!       vy(jx,jy,jz) = vy(jx,jy,jz)*ratio
!       vz(jx,jy,jz) = vz(jx,jy,jz)*ratio
!!!!!!!!!!! This part has been replaced by the call to rescale_spec !!!!!!

!...compute contribution to the new energy spectrum
!!!! The spectrum after rescaling.
        Rex =  real(vx(jx,jy,jz))
        Imx = aimag(vx(jx,jy,jz))
        Rey =  real(vy(jx,jy,jz))
        Imy = aimag(vy(jx,jy,jz))
...
subroutine geninit(idum)
  use global
  implicit none

  real(8), dimension(nx/2) :: cntem, cntep, cnthm, cnthp, fpk, fmk
  complex(8), dimension(nx/2+1,ny,nz) :: ap, am

  complex(8), parameter :: eye = (0.d0, 1.d0)

  complex(8) :: hpx, hpy, hpz, hmx, hmy, hmz
  real(8) :: tt, t2, k2d, spec3d, heli_spec3d
  real(8) :: kx, ky, kz, kyky, kzkz, k22
  integer :: ll, jx, jy, jz, idum
  ...
  do jz = 1,nz
    ...
    do jy = 1,ny
      ...
      do jx = 1, nx/2 + 1
        ...
        t2 = two_Pi*t2
        am(jx,jy,jz) = cmplx(dcos(t2),dsin(t2),kind=8)
    
        t2 = dble(ran2(idum))
        if (t2 <= 1.d-10) t2 = 1.d-10
        t2 = two_Pi*t2
        ap(jx,jy,jz) = cmplx(dcos(t2),dsin(t2),kind=8)


        kx = dble(jx - 1)

        k22 = sqrt(kx*kx + kyky + kzkz)
        if (k22 < 1.d-20) cycle   !!! skip the zero mode

        ll=floor(k22+.5)  !!!! ll-.5 =< k22 < ll+.5
        if (ll .ge. 1 .and. ll .le. nx/2) then  
             if ( jx == 1 ) then 
                 t2 =.5*ap(jx,jy,jz)*conjg(ap(jx,jy,jz)) !!! kx = 0 plane
                 tt =.5*am(jx,jy,jz)*conjg(am(jx,jy,jz)) 
             else 
                 t2 = ap(jx,jy,jz) * conjg(ap(jx,jy,jz)) !!! kx /=0 
                 tt = am(jx,jy,jz) * conjg(am(jx,jy,jz))
             end if
             cntep(ll)=cntep(ll)+t2
             cntem(ll)=cntem(ll)+tt
             cnthp(ll)=cnthp(ll)+k22*t2
             cnthm(ll)=cnthm(ll)+k22*tt
        end if
  ...
  do jx=1,nx/2
    fpk(jx) = sqrt(spec3d(dble(jx))*cnthm(jx)+.5*heli_spec3d(dble(jx))*cntem(jx)) &
             /sqrt(cntep(jx)*cnthm(jx)+cnthp(jx)*cntem(jx))
    fmk(jx) = sqrt(max(spec3d(dble(jx))*cnthp(jx)-.5*heli_spec3d(dble(jx))*cntep(jx),0.)) &
             /sqrt(cntem(jx)*cnthp(jx)+cnthm(jx)*cntep(jx))
  end do
  !TODO: using analytical spectra. Can we use DNS spectra, no data for higher nx though????
  ...
  do jz=1,nz
    ...
    do jy=1,ny
      ...
      ! on the kx axis, another definition for hm and hp is used.
      do jx=1,nx/2+1
        kx = dble(jx - 1)
        ...
        ll=floor(k22+.5)
        if (ll .ge. 1 .and. ll .le. nx/2) then
            am(jx,jy,jz)=am(jx,jy,jz)*fmk(ll)
            ap(jx,jy,jz)=ap(jx,jy,jz)*fpk(ll)
        else
            am(jx,jy,kz) = 0.d0
            ap(jx,jy,kz) = 0.d0
        end if
 
        if (jy .eq. 1 .and. jz .eq. 1) then  
            k2d=max(sqrt(kx*kx+kyky),tiny_real)
            
            hpx=(sqrt(2.d0)/2.d0)*(-ky/k2d-eye*kx*kz/k22/k2d)
            hpy=(sqrt(2.d0)/2.d0)*(kx/k2d-eye*ky*kz/k22/k2d)
            hpz=(sqrt(2.d0)/2.d0)*eye*(kyky+kx*kx)/k22/k2d
        else
            k2d=sqrt(kzkz+kyky)
            
            hpx=(sqrt(2.d0)/2.d0)*eye*(kyky+kzkz)/k22/k2d
            hpy=(sqrt(2.d0)/2.d0)*(-kz/k2d-eye*kx*ky/k22/k2d)
            hpz=(sqrt(2.d0)/2.d0)*(ky/k2d-eye*kz*kx/k22/k2d)
        end if
        ...
        vx(jx,jy,jz)=ap(jx,jy,jz)*hpx+am(jx,jy,jz)*hmx
        vy(jx,jy,jz)=ap(jx,jy,jz)*hpy+am(jx,jy,jz)*hmy
        vz(jx,jy,jz)=ap(jx,jy,jz)*hpz+am(jx,jy,jz)*hmz
...
subroutine rescale_spec
  use global
  implicit none

  real(8), dimension(nx/2) :: cntem, cntep, cnthm, cnthp, fmk, fpk
  complex(8), dimension(nx/2+1,ny,nz) :: ap, am
  ...
  write(*,*) 'I am in rescale_spec'

  cntem = 0d0; cntep =0d0; cnthm = 0d0; cnthp = 0d0
  do jz = 1,nz
    if (jz == nzhp) cycle
    kz = jz - 1
    if (jz > nzh) kz = kz - nz
    ...
    do jy = 1,ny
      ...
      do jx=1,nx/2+1
        ...
        ap(jx,jy,jz) = vx(jx,jy,jz) * hmx + vy(jx,jy,jz) * hmy + vz(jx,jy,jz) * hmz
        am(jx,jy,jz) = vx(jx,jy,jz) * hpx + vy(jx,jy,jz) * hpy + vz(jx,jy,jz) * hpz
  ...
  !write(*,*) 'I have finished ap am cntem etc'
  ...
  !write(*,*) 'I have finished fpk and fmk'
  ...
  do jz=1,nz
    ...
    do jy=1,ny
      ...
      !write(*,*) 'middle jy loop', jy
      do jx=1,nx/2+1
        kx = dble(jx - 1)
        k22=sqrt(kx * kx + kyky + kzkz)
        !write(*,*) 'inner jx loop', jx
        ...
        !write(*,*) 'after vx vy vz'

      end do
    end do
  end do

  write(*,*) 'I have finished rescaling vx vy vz'

  call symmetrize(vx)
  call symmetrize(vy)
  call symmetrize(vz)

end subroutine rescale_spec

subroutine symmetrize(c)
  use global
  implicit none

  complex(8), dimension(nx/2+1,ny,nz), intent(inout) :: c

  integer :: iy,iz

! zero ky=-ny/2 modes
  c(:,ny/2+1,:) = (0.0d0,0.0d0)
! zero kz=-nz/2 modes
  c(:,:,nz/2+1) = (0.0d0,0.0d0)
! zero kx=nx/2 modes
  c(nx/2+1,:,:)=(0.0d0,0.0d0)
! zero kx=ky=kz=0 mode
  ...
  do iy=1,ny
  do iz=nz/2+2,nz
  ! using MOD to handle iy=1; for iy>1, MOD(...)+1 = ny+2-iy
  c(1,iy,iz) = CONJG(c(1,MOD(ny+1-iy,ny)+1,nz+2-iz))
  enddo
  enddo

  ! imposing the constraint on the line kx=kz=0.
  do iy=ny/2+2,ny
  c(1,iy,1) = CONJG(c(1,ny+2-iy,1))
  enddo
