program cml
  ...
  if (ll .ne. 4) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./cml-09022009.x nini tmax idum ireal'
...
subroutine drift(aij, driftaij)
  ...
  real(sp), parameter :: gam = 0.08, beta = -1.
  real(sp), dimension(three, three) :: aij, driftaij

  real(sp), dimension(three, three) :: a2ij, aijaji, id, ajiaij, hij
  real(sp) :: tra2
  ...
  ajiaij = transpose(aijaji)

  hij = -(tra2/3.) * (id - (aij + transpose(aij))*gam) - beta * (ajiaij - aijaji) &
        + beta * gam * (matmul(transpose(aij), ajiaij) - matmul(transpose(aij), aijaji) &
        + matmul(transpose(aij), a2ij) - matmul(aijaji, aij) )

  driftaij = - a2ij - aij  - hij
