program ptvort
  ...
  real(sp) :: ignore_me, delta_c, dt, time, meancurv, curvp, meanalfcurv
  real(sp) :: ww, wxp, wyp, wzp, meanalpha, alphap, const, meanalfww
  ...
  real(sp),    allocatable, dimension(:)     :: kx, ky, kz, alfp
  ...
  if (nx .ne. 6) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./pt-vort.x nx filelist ndel nprtcle dt prefix'
  ...
  allocate(xp(nprtcle), yp(nprtcle), zp(nprtcle), alfp(nprtcle) )
  ...
  open(25, file = 'ww-'//flnm(1:len_trim(flnm))//'.dat')
...
!    if ( nfile .eq. 0 ) then
!      ww0 =       sum( real( wz(1:lx,:,:) * conjg(wz(1:lx,:,:)) ) )
!      ww0 = ww0 + sum( real( wy(1:lx,:,:) * conjg(wy(1:lx,:,:)) ) )
!      ww0 = ww0 + sum( real( wx(1:lx,:,:) * conjg(wx(1:lx,:,:)) ) )
!
!      ww0 =  ww0 / (nx * ny * nz) 
!      write(*,*) 'Estimated mean ww:', ww0 
!    end if
    ...
    do ip=1, nprtcle
      ...
      call value(wxr,wxp,lhnode,bg,nx,ny,nz)
      call value(wyr,wyp,lhnode,bg,nx,ny,nz)
      call value(wzr,wzp,lhnode,bg,nx,ny,nz)

      call value(alpha,alphap,lhnode,bg,nx,ny,nz)

      ignore_me = wxp * wxp + wyp * wyp + wzp * wzp

      ww = ww + ignore_me

      meanalpha = meanalpha + alphap

      meanalfww = meanalfww + ignore_me * alphap
    ...
    do ip=1, nprtcle
      ...
      meanalfcurv = meanalfcurv + alfp(ip) * curvp

    end do
    meancurv = meancurv / nprtcle
    meanalfcurv = meanalfcurv / nprtcle 

    write(25, '(10E15.4)') time, ww, meanalpha, meancurv, -meanalfcurv, meanalfww
    time = time + dt
  ...
  deallocate(ux, uy, uz, wx, wy, wz, wxr, wyr, wzr, kx, ky, kz, g, xp, yp, zp)
