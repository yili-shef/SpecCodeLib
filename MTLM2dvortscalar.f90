MODULE global
    ...
    complex(8), dimension(nt/2 + 1,nt) :: vx,vy,theta,vort
 
 
    real(8) E(lh-2),Et(lh-2),eta,alpha
    real(8) hx,hy,CFL,max_k2,resc_factor,thetaresc_factor,epsilon,epsl,tiny_real
...
PROGRAM MTLM_uT
    ...
    complex(8), dimension(nbnd/2*256 + 1, nbnd*256 ) :: vx2048,vy2048
    complex(8), dimension(nbnd/2*128 + 1, nbnd*128 ) :: vx1024,vy1024
    complex(8), dimension(nbnd/2*64 + 1, nbnd*64 ) :: vx512,vy512
    ...
    complex(8), dimension(nbnd/2*256 + 1, nbnd*256 ) :: theta2048
    complex(8), dimension(nbnd/2*128 + 1, nbnd*128 ) :: theta1024
    complex(8), dimension(nbnd/2*64 + 1, nbnd*64 ) :: theta512
    ...
    !...for reduced vorticity field
    complex(8), dimension(nbnd/2*256 + 1, nbnd*256 ) :: vort2048
    complex(8), dimension(nbnd/2*128 + 1, nbnd*128 ) :: vort1024
    complex(8), dimension(nbnd/2*64 + 1, nbnd*64 ) :: vort512
    complex(8), dimension(nbnd/2*32 + 1, nbnd*32 ) :: vort256
    complex(8), dimension(nbnd/2*16 + 1, nbnd*16 ) :: vort128
    complex(8), dimension(nbnd/2*8  + 1, nbnd*8  ) :: vort64
    complex(8), dimension(nbnd/2*4  + 1, nbnd*4  ) :: vort32
    complex(8), dimension(nbnd/2*2  + 1, nbnd*2  ) :: vort16
    complex(8), dimension(nbnd/2    + 1, nbnd    ) :: vort08
    ...
    integer(8) FTvort,IFTvort
    integer(8) FTvort2048,IFTvort2048
    integer(8) FTvort1024,IFTvort1024
    integer(8) FTvort512,IFTvort512
    integer(8) FTvort256,IFTvort256
    integer(8) FTvort128,IFTvort128
    integer(8) FTvort64,IFTvort64
    integer(8) FTvort32,IFTvort32
    integer(8) FTvort16,IFTvort16
    integer(8) FTvort08,IFTvort08
    
    complex(8) a,b,t,vxx,vyy
    
    real(8) d,mag,ratio,wn,t2,ur,tadv, thetarmsl
    real(8) spec2d,kx,k22,s2,upl,spec2d_scalar
    ...
    Int0kmax = s2
    urms0 = sqrt(Int0kmax)
    ! The spectrum and urms0 are to be maintained during the mapping. 

    ! The Ekman frictional coefficient.
    alpha = diss / Int0kmax / 2.
    
    s2 = 0.5d0 * spec2d_scalar(1.0d0)
    do jx = 2, lh-2
      s2 = s2 + 0.5d0*(spec2d(dble(jx-1)) + spec2d(dble(jx)))
    end do
    Int0kmaxscalar = s2
    ...
    call dfftw_plan_dft_c2r_2d(IFTvort,nnr,nnr,vort,vort,FFTW_MEASURE)
    ...
    call dfftw_plan_dft_c2r_2d(FTvort,nnr,nnr,vort,vort,FFTW_MEASURE)

    nnr = nbnd*256
    print *,nnr
    call dfftw_plan_dft_c2r_2d(IFTvx2048,nnr,nnr,vx2048,vx2048,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(IFTvy2048,nnr,nnr,vy2048,vy2048,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(IFTth2048,nnr,nnr,theta2048,theta2048,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(IFTvort2048,nnr,nnr,vort2048,vort2048,FFTW_MEASURE)
    
    call dfftw_plan_dft_r2c_2d(FTvx2048,nnr,nnr,vx2048,vx2048,FFTW_MEASURE)
    call dfftw_plan_dft_r2c_2d(FTvy2048,nnr,nnr,vy2048,vy2048,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(FTth2048,nnr,nnr,theta2048,theta2048,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(FTvort2048,nnr,nnr,vort2048,vort2048,FFTW_MEASURE)
    
    nnr = nbnd*128
    print *,nnr
    call dfftw_plan_dft_c2r_2d(IFTvx1024,nnr,nnr,vx1024,vx1024,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(IFTvy1024,nnr,nnr,vy1024,vy1024,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(IFTth1024,nnr,nnr,theta1024,theta1024,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(IFTvort1024,nnr,nnr,vort1024,vort1024,FFTW_MEASURE)
    
    call dfftw_plan_dft_r2c_2d(FTvx1024,nnr,nnr,vx1024,vx1024,FFTW_MEASURE)
    call dfftw_plan_dft_r2c_2d(FTvy1024,nnr,nnr,vy1024,vy1024,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(FTth1024,nnr,nnr,theta1024,theta1024,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(FTvort1024,nnr,nnr,vort1024,vort1024,FFTW_MEASURE)
    
    
    nnr = nbnd*64
    print *,nnr
    call dfftw_plan_dft_c2r_2d(IFTvx512,nnr,nnr,vx512,vx512,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(IFTvy512,nnr,nnr,vy512,vy512,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(IFTth512,nnr,nnr,theta512,theta512,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(IFTvort512,nnr,nnr,vort512,vort512,FFTW_MEASURE)
    
    call dfftw_plan_dft_r2c_2d(FTvx512,nnr,nnr,vx512,vx512,FFTW_MEASURE)
    call dfftw_plan_dft_r2c_2d(FTvy512,nnr,nnr,vy512,vy512,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(FTth512,nnr,nnr,theta512,theta512,FFTW_MEASURE)
    call dfftw_plan_dft_c2r_2d(FTvort512,nnr,nnr,vort512,vort512,FFTW_MEASURE)
    ...
    call dfftw_plan_dft_c2r_2d(IFTvort256,nnr,nnr,vort256,vort256,FFTW_MEASURE)
    ...
    call dfftw_plan_dft_c2r_2d(FTvort256,nnr,nnr,vort256,vort256,FFTW_MEASURE)
    ...
    call dfftw_plan_dft_c2r_2d(IFTvort128,nnr,nnr,vort128,vort128,FFTW_MEASURE)
    ...
    call dfftw_plan_dft_c2r_2d(FTvort128,nnr,nnr,vort128,vort128,FFTW_MEASURE)
    ...
    call dfftw_plan_dft_c2r_2d(IFTvort64,nnr,nnr,vort64,vort64,FFTW_MEASURE)
    ...
    call dfftw_plan_dft_c2r_2d(FTvort64,nnr,nnr,vort64,vort64,FFTW_MEASURE)
    ...
    call dfftw_plan_dft_c2r_2d(IFTvort32,nnr,nnr,vort32,vort32,FFTW_MEASURE)
    ...
    call dfftw_plan_dft_c2r_2d(FTvort32,nnr,nnr,vort32,vort32,FFTW_MEASURE)
    ...
    call dfftw_plan_dft_c2r_2d(IFTvort16,nnr,nnr,vort16,vort16,FFTW_MEASURE)
    ...
    call dfftw_plan_dft_c2r_2d(FTvort16,nnr,nnr,vort16,vort16,FFTW_MEASURE)
    ...
    call dfftw_plan_dft_c2r_2d(IFTvort08,nnr,nnr,vort08,vort08,FFTW_MEASURE)
    ...
    call dfftw_plan_dft_c2r_2d(FTvort08,nnr,nnr,vort08,vort08,FFTW_MEASURE)
    ...
    !...Calculate random velocity and passive scalar field.  abs(v(k))=sqrt(2E(k)/N(k))
    do jy = 1,ny
      if (jy == nyhp) cycle
      ky = jy - 1
      if (jy > nyh) ky = ky - ny
      ...
      do jx = 1,nth
        ...
        if(k22 < max_k2) then
          ...
          t2 = two_Pi*t2
          b = cmplx(dcos(t2),dsin(t2),kind=8)
          d = sqrt(2.d0*spec2d_scalar(wn)/dble(num(ind)))
          theta(jx,jy) = b*d
      ...
      do jx = 1, nth
        ...
        if (k22 < max_k2) then
          ...
          if (ind < nth) then ! nth = half of nt
            ...
            ! Vorticity
            vort(jx,jy) = (0.d0,1.d0) * ( kx * vy(jx,jy) - ky * vx(jx,jy) )
            ...
            if (Et(ind) < 2.d-38) then
               ...
               ratio = sqrt(spec2d_scalar(wn)/Et(ind))
            end if    
            theta(jx,jy) = theta(jx,jy) * ratio

            Rex =  real(theta(jx,jy))
            Imx = aimag(theta(jx,jy))
            ss = Rex*Rex + Imx*Imx
            if (kx < 1.d-20) ss = ss * 0.5d0
            if (ss < tiny_real) ss = 0.d0
    ...
    vx = vx * resc_factor
    vy = vy * resc_factor
    vort = vort * resc_factor

    !...second rescale to match the variance
    s2 = 0.5d0 * Etloc(1)
    do jx = 2,lh-2
      s2 = s2 + 0.5d0*(Etloc(jx-1) + Etloc(jx))
    end do
    thetarmsl = sqrt( 2.d0*s2  )
    resc_factor = thetarms0/thetarmsl
    ...
    open(unit=21, file='./out/theta0.dat', form='unformatted')
      write(21) theta
    close(unit=21)
    ...
    call apply_MTLM(vx08,vy08,vort08,theta08,nbnd + 2,                         &
    IFTvx08,IFTvy08,IFTvort08,IFTth08,FTvx08,FTvy08,FTvort08,FTth08)
    
    call apply_MTLM(vx16,vy16,vort16,theta16,nbnd*2+ 2,                         &
    IFTvx16,IFTvy16,IFTvort16,IFTth16,FTvx16,FTvy16,FTvort16,FTth16)
    
    call apply_MTLM(vx32,vy32,vort32,theta32,nbnd*4+ 2,                          &
    IFTvx32,IFTvy32,IFTvort32,IFTth32,FTvx32,FTvy32,FTvort32,FTth32)
    
    call apply_MTLM(vx64,vy64,vort64,theta64,nbnd*8 + 2,                          &
    IFTvx64,IFTvy64,IFTvort64,IFTth64,FTvx64,FTvy64,FTvort64,FTth64)
    
    call apply_MTLM(vx128,vy128,vort128,theta128,nbnd*16 + 2,IFTvx128,IFTvy128,    &
    IFTvort128,IFTth128,FTvx128,FTvy128,FTvort128,FTth128)
    
    call apply_MTLM(vx256,vy256,vort256,theta256, nbnd*32 + 2,IFTvx256,IFTvy256,    &
    IFTvort256,IFTth256,FTvx256,FTvy256,FTvort256,FTth256)
    
    call apply_MTLM(vx512,vy512,vort512,theta512, nbnd*64 + 2,IFTvx512,IFTvy512,    &
    IFTvort512,IFTth512,FTvx512,FTvy512,FTvort512,FTth512)
    
    call apply_MTLM(vx1024,vy1024,vort1024,theta1024, nbnd*128 + 2,IFTvx1024,IFTvy1024,    &
    IFTvort1024,IFTth1024,FTvx1024,FTvy1024,FTvort1024,FTth1024)
    
    call apply_MTLM(vx2048,vy2048,vort2048,theta2048, nbnd*256 + 2,IFTvx2048,IFTvy2048,    &
    IFTvort2048,IFTth2048,FTvx2048,FTvy2048,FTvort2048,FTth2048)
    
    !...the last scale is direct (no filtered fields):
    
    write(*,"('using epsilon value =',ES25.15)") epsilon
    write(*,"('========= The last scale ===============')") 
    ...
    do while (total_adv .le. time_scale)
      ...
      call dfftw_execute(IFTvort)
      write(*,"('53/',$)") 
    
      !... Apply MTLM on disk
      call MTLM(vx,vy,theta,ld,tadv)
      ...
      write(*,"('54/',$)") 
      call dfftw_execute(FTvort)
      vort = vort/dble(nx*ny)
      write(*,"('55/',$)") 
    
      vort(lh,1:ny) = (0.d0,0.d0)      !Oddballs
      vort(1:lh,nyhp) = (0.d0,0.d0)
      vort(1,1) = (0.d0,0.d0)   !<u_i>=0
    ...
    !++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      do jy = 1,ny
        ...
        do jx = 1,lhm
          ...
          if (k22 < max_k2) then
            vx(jx,jy) =  (0.d0, 1.d0) * ky * vort(jx,jy) / k22
            vy(jx,jy) = -(0.d0, 1.d0) * kx * vort(jx,jy) / k22
      ...
      s2 = sqrt(s2*2.d0)
      theta = theta * thetarms0 / s2
      write(*,*) 'rescale factor for scalar: ', thetarms0/s2
      ...
      do jx = 1,lhm
        ...
        if (k22 < max_k2) then
          ...
          if (ind < lh - 1) then
            ...
            ! Et(k) is calculated in Subroutine spectrum
            if (Et(ind) < 2.d-38) then
               ratio = 0.0d0
            else  
               ratio = sqrt(spec2d_scalar(wn)/Et(ind))
    ...
    open(unit=21, file='./out/uy'//cmd(1:len_trim(cmd))//'.dat', form='unformatted')
      write(21) vy
    close(unit=21)
    open(unit=21, file='./out/theta'//cmd(1:len_trim(cmd))//'.dat', form='unformatted')
      write(21) theta
    close(unit=21)
...
subroutine spectrum(iu,name, output)
    ...
    real(8) s2,spec2d,kx,k22,upl, spec2d_scalar
    real(8) Rex,Imx,Rey,Imy,ss

    
    
    write(*,"(/,'calculating energy and scalar spectrum...',/)")
    ...
    upl = sqrt(2.d0*s2)
    !thetaresc_factor = thetarms0/upl   
    
    if (iu > 0) then

      open(unit = iu, file = name, status = 'unknown')

        write(iu, '('' # Statistics of the generated velocity and scalar fields'')')
        write(iu,"('#      k      E(k) discrete   analytical, Et(k) discrete analytical')")

        do jx = 1,nth - 1
          write(iu,"(f11.5,4(es16.6))") real(jx), E(jx), spec2d(dble(jx)), Et(jx), &
          spec2d_scalar(dble(jx))   
        end do
...
real(8) function spec2d_scalar(rk)
!****** 3-D spectrum function for passive scalar********
use global
implicit none
real(8) rk,kl
kl = rk * lq
spec2d_scalar = Cq * qdiss * diss**(-1.d0/3.d0) * rk**(-5.d0/3.d0)   &
                   * (kl/sqrt(kl*kl + clq))**(5.d0/3.d0 + p0)        &
                   * dexp(-1.5d0*Cq*(rk*etaq)**4.d0/3.d0)
end function spec2d_scalar




Subroutine apply_MTLM(vxred,vyred,vortred,thetared,ldr,IFTvxred,IFTvyred,   &
                      IFTvortred,IFTthred,FTvxred,FTvyred,FTvortred,FTthred)
    !...Apply the MMLM with:
    ...
    complex(8), dimension(ldr/2,ldr-2) :: vxred,vyred,vortred,thetared
    real(8), dimension(lh-2) :: Eloc, Etloc
    integer(8) IFTvxred,IFTvyred,IFTvortred,FTvxred,FTvyred,FTvortred,IFTthred,FTthred
    real(8) hxr,wn,spec2d,ratio,kx,k22,ur,tn,thetar,time_scale, spec2d_scalar
    real(8) u1,u2,v1,v2,total_adv,tadv,Int0kmaxr,urb,s2,thetarb, Int0kmaxscalarr
    real(8) Rex,Imx,Rey,Imy,ss,upl,thetapl
    ...
    loop_jyr: do jyr = 1,nyrh
      ...
      do jxr = 1,lhrm
        ...
        if (ind < Sup_shell) then
          ...
          u1 =  real( thetared(jxr,jyr) )
          u2 = aimag( thetared(jxr,jyr) )
          u1 = u1*u1 + u2*u2
          if (kx < 1.d-20) u1 = u1 * 0.5d0
          thetar = thetar + u1
    ...
    thetar = sqrt(thetar) ! rms of filtered scalar field
    ...
    s2 = 0.5d0 * spec2d_scalar(1.0d0)   
    do jx=2,lhr-1
      s2 = s2 + 0.5d0*(spec2d_scalar(dble(jx-1)) + spec2d_scalar(dble(jx)))
    end do
    Int0kmaxscalarr = s2
    thetarb = sqrt(Int0kmaxscalarr/Int0kmaxscalar) * thetarms0
    ! The thetarb is taken as a portion of the total rms. The portion is calculated from the
    ! variance ratio.
   
    
    CFL = 1. !TODO: specify CFL instead?
    ! CFL = tadv/tn
    write(  *,"('       CFL = ',ES12.5)") CFL
    write(911,"('       CFL = ',ES12.5)") CFL  
    ...
    do while (total_adv .le. time_scale) 
      ...
      call dfftw_execute(IFTvortred)
      write(*,"('53/',$)") 
      ! vx vy and theta are real 
    
      ! advance one step 
      call MTLM(vxred,vyred,vortred,thetared,ldr,tadv)
      ...
      ! vorticity to complex
      call dfftw_execute(FTvortred)
      vortred = vortred/dble(nxr*nyr)

      !....vorticity field
      vortred(lhr,1:nyr) = (0.d0,0.d0)      !Oddballs
      vortred(1:lhr,nyr/2+1) = (0.d0,0.d0)
      vortred(1,1) = (0.d0,0.d0)   !<vort>=0
      ...
      do jyr = 1,nyr
        ...
        do jxr = 1,lhr-1
          ...
          if (ind < Sup_shell) then
              vxred(jxr,jyr) =   (0.d0, 1.d0) * ky * vortred(jxr,jyr) / k22
              vyred(jxr,jyr) = - (0.d0, 1.d0) * kx * vortred(jxr,jyr) / k22
              ...
              Rex =  real(thetared(jxr,jyr))
              Imx = aimag(thetared(jxr,jyr))
              ur = Rex * Rex + Imx * Imx
              if (kx < 1.d-20) ur = ur * 0.5d0
              if (ur < tiny_real) ur = 0.d0
              Etloc(ind) = Etloc(ind) +  ur
      ...
      vxred = vxred * urb / s2
      vyred = vyred * urb / s2
      vortred = vortred * urb / s2
      write(*,*) 'YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY'
      write(*,*) 'urb = ', urb, 's2 = ', s2
      ...
      s2 = sqrt(s2*2.d0)
      thetared = thetared * thetarb / s2
      write(*,*) 'rescale factor for scalar: ', thetarb/s2
    ...
    loop_jyr5: do jyr = 1,nyrh
       ...
       do jxr = 1,lhrm
          ...
          if (ind < Sup_shell) then                
             ...
             vort(jxr,jyr) = vortred(jxr,jyr)
             theta(jxr,jyr) = thetared(jxr,jyr)
          end if
       end do  
    end do loop_jyr5
    ...
    loop_jyr6: do jyr = nyrh+1,nyr
       ...
       do jxr = 1,lhrm
          ...
          if (ind < Sup_shell) then
            ...
            vort(jxr,jy) = vortred(jxr,jyr)
            theta(jxr,jy) = thetared(jxr,jyr)
          end if
       end do
    end do loop_jyr6
      ...
      do jx = 1,lhm
        ...
        if (k22 < max_k2) then
          ...
          if (ind < nth) then
            ...
            if (Et(ind) < 2.d-38) then  !!! E has been updated in subroutine spectrum
               ratio = 0.0d0
            else  
               ratio = sqrt(spec2d_scalar(wn)/Et(ind))
            end if    
    ...
    !...second rescale to match the kinetic energy and variance
    write(*,"('Rescaling the energy and scalar spectrum (2/2)')")
    ...
    thetapl = sqrt(2.d0*s2)
    resc_factor = thetarms0/thetapl
    print *, 'thetarms0 = ', thetarms0
    print *, 'thetapl = ', thetapl
    print *,'theta resc= ',resc_factor
...
SUBROUTINE MTLM(vxred,vyred,vortred,thetared,ldr,tadv)
    ...
    integer, intent(in) :: ldr
    complex(8), dimension(ldr/2,ldr-2) :: vxred,vyred,vortred,thetared
    real(8), dimension(ldr-2,ldr-2) :: ugrid,vgrid,Swf,vortgrid,thetagrid
    
    integer, dimension(ldr-2) :: is,ie
    integer, dimension(ldr-2) :: js,je
    
    real(8) dt,maxd,wf,hxr,hyr,R,minwf,tadv
    real(8) ua1,va1,ua2,va2,ir,jr,dirl2,dirh2,djrl2,djrh2, theta1, theta2, vort1, vort2
    integer il,ih,jl,jh,ia,ja,ia1,ia2,i,j,nxr,nyr
    real(8) swfs,Swfu,Swfv,R2,Ra
    real(8) Swftheta, Swfvort
    ...
    !****(1) 
       ...
       if (wf > minwf) then
         ...
         vortgrid(il,jl) = vortgrid(il,jl) + wf * vort1
       ...
       if (wf > minwf) then
         ...
         vortgrid(il,jh) = vortgrid(il,jh) + wf * vort1
       ...
       if (wf > minwf) then
         ...
         vortgrid(ih,jl) = vortgrid(ih,jl) + wf * vort1
       ...
       if (wf > minwf) then
         ...
         vortgrid(ih,jh) = vortgrid(ih,jh) + wf * vort1
    ...
    !****(2) 
       ...
       vortgrid(il,jl) = vortgrid(il,jl) + wf * vort2
       ...
       vortgrid(il,jh) = vortgrid(il,jh) + wf * vort2
       ...
       vortgrid(ih,jl) = vortgrid(ih,jl) + wf * vort2
       ...
       vortgrid(ih,jh) = vortgrid(ih,jh) + wf * vort2
    ...
    do i = 1,nxr
      if (Swf(i,j) .ge. tiny_real) then
         ...
         vortgrid(i,j) = vortgrid(i,j) / Swf(i,j) 
