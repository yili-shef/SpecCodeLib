program findhwpoints
    ...
    integer, parameter :: nx = 256, ndel = 0
    real(sp), parameter ::  nrms = 1./3.
    ...
    open(35, file = 'findlo.list')
        ...
        do while ( .not. eof(35) )
            ...
            do ii=1,nx
              ...
              if (ignore_me .le. nrms * rmsw)  then
                ip = ip + 1
                xp(ip) = (ii-1) * 2 * pi / nx
                yp(ip) = (jj-1) * 2 * pi / ny
                zp(ip) = (kk-1) * 2 * pi / nz
            ...
            str = './out/'//strfn(1:len_trim(strfn))//'lowo-'//str(1:len_trim(str))
