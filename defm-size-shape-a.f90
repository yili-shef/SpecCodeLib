program defmsizeshape
  ...
  real(dp) :: kin, ix, iy, iz, angularmom, meanaijaij, meantrcc, trcctmp
  real(dp) :: sstar, gf, iic
  
  if (iargc() .ne. 3) then
          write(*,*)
          write(*,*) ' >>>>>> Wrong number of arguments <<<<<<'
          write(*,*) 
          write(*,*) ' Usage: ./defm-size-shape-a.x nx filelist prefix'
  ...
  open(27, file = 'size-cnd-shape-a-'//trim(flnm)//'.dat')
  write(27, '("# sstar  pdfgf  cndsize   ")') 
  ...
  do while ( .not. eof(30) )
    ...
    do ii = 1, nx
      ...
      !iic = .5_sp * (trcctmp**2 - evalues(1)**2 - evalues(2)**2 - evalues(3)**2) 
      iic =  evalues(1)**2 + evalues(2)**2 + evalues(3)**2 
      gf = trcctmp**2 / (3._sp * iic)   !gf falls in 0 < gf < 1
    ...
    write(*,*) 'normalization of pdf gf: ' sum(pdfsstar)*const
    pdfsstar = pdfsstar * const / binw

    cndsize = cndsize/meantrcc

    write(27, '( "# index = ", I6)') nfile
    write(27, '( "# meantrcc = ", E18.5)') meantrcc
    do ll = 1, npnt
      write(27, '(15E15.3)') (ll - 0.5_sp) * binw, pdfsstar(ll), cndsize(ll)
    end do
    write(27, *)
    write(27, *)
  ...
  write(*,*) 'defm-size-shape-a.x done.'
