program rcmmore
  ...
  real(dp), dimension(npdf) :: poos, plnaijaij
  ...
  if (ll .ne. 4) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./cmmodel-more-pdf.x nfiles nstart nreal ifirst'
  ...
  str = 'more-pdf-'//strreal(1:len_trim(strreal))//'reals.dat'
  ...
  do ll = nstart, nfiles + nstart -1
    ...
    do ii = 1, npdf
      tmp = -bnd + (ii-.5) * binw
      write(21,'(15E15.4)') tmp, poos(ii), plnaijaij(ii)
    end do
