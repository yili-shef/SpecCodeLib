program spectral_dns
  ...
  do while (time .le. timemax)
    ...
    k2_e = (kx*real(wx,sp) + ky*real(wy,sp) + kz*real(wz,sp))/k2
    wx = cmplx(real(wx,sp) - kx*k2_e, aimag(wx))
    wy = cmplx(real(wy,sp) - ky*k2_e, aimag(wy))
    wz = cmplx(real(wz,sp) - kz*k2_e, aimag(wz))

    k2_e = (kx*aimag(wx) + ky*aimag(wy) + kz*aimag(wz))/k2
    wx = cmplx(real(wx,sp), aimag(wx) - kx*k2_e)
    wy = cmplx(real(wy,sp), aimag(wy) - ky*k2_e)
    wz = cmplx(real(wz,sp), aimag(wz) - kz*k2_e)
