subroutine sgsexpa (vx,vy,vz,t11,t12,t13,t22,t23,t33,kx,ky,kz,lx1,ly,lz,nx,ny,nz,delta)
  use mconstant
  use mfftwplan3d
  implicit none
  ...
  do ii=1,nx
    ...
    if ( mod(ii,2) .eq. 1) then
      ...
      t11(ll,jj,kk)=cmplx(RA(1,1),A(1,1)) * const
      t12(ll,jj,kk)=cmplx(RA(1,2),A(1,2)) * const
      t13(ll,jj,kk)=cmplx(RA(1,3),A(1,3)) * const
      t22(ll,jj,kk)=cmplx(RA(2,2),A(2,2)) * const
      t23(ll,jj,kk)=cmplx(RA(2,3),A(2,3)) * const
      t33(ll,jj,kk)=cmplx(RA(3,3),A(3,3)) * const
