program jpdfcurvtor
  ...
  real(dp), dimension(npnt) :: pdfcurv, pdftor
  real(dp), dimension(npnt, npnt) :: jpdfct
  ...
  write(*,*) 
  write(*,'(''>>>>>> Mean rms and PDFs of Curvature and Torsion of vortex lines <<<<<<'')')
  write(*,*)
  ll=iargc()
  if (ll .ne. 3) then
          write(*,*)
          ...
          write(*,*) 
          write(*,*) ' Usage: ./jpdf-ct.x nx filelist ndel'
  ...
  do while ( .not. eof(20) )
    ...
    do ii = 1, nx
      ...
      ll = floor( (delta_c + bndtor) / binwtor) + 1

      if ( mm .ge. 1 .and. mm .le. npnt .and. ll .ge. 1 .and. ll .le. npnt) then
          pdfcurv(mm) = pdfcurv(mm) + 1
          pdftor(ll)  = pdftor(ll)  + 1
          jpdfct(mm,ll) = jpdfct(mm,ll) + 1
      end if
  ...
  pdftor = pdftor / nfile * const
  write(*,*) 'Check normalizationof pdftor:', sum(pdftor)

  pdftor = pdftor / binwtor

  covct = covct / nfile * const
  covct = covct - meancurv * meantor
  covct = covct / (rmscurv * rmstor) 

  jpdfct = jpdfct / nfile * const
  jpdfct = jpdfct / (binwcurv * binwtor)

  ! Ouput format for gnuplot
  open(15,file='pdf-curv-tor'//str(1:len_trim(str)))
    ...
    write(15,*) '# Zone T= "pdfs of curvature and torsion", i=', npnt, ', F=point'
    do ii=1,npnt
      write(15,'(15E15.5)') (ii-.5) * binwcurv * meancurv0 / meancurv, pdfcurv(ii)*meancurv/meancurv0, &
                            (-bndtor + (ii-.5) * binwtor)*rmstor0/rmstor, pdftor(ii)*rmstor/rmstor0
    end do
  close(15)

  ! output format for gnuplot
  open(15,file='jpdf-ct'//str(1:len_trim(str)))
    write(15,*) '# Zone T= "jpdf curv tor", i=', npnt, ', j=', npnt, ', F=point'
    do ii = 1, npnt
    do jj = 1, npnt
      write(15,'(15E15.5)') (ii-.5) * binwcurv * meancurv0 / meancurv, &
              ( -bndtor + (jj-.5) * binwtor) * rmstor0 / rmstor, & 
              jpdfct(ii,jj) * rmstor * meancurv / rmstor0 / meancurv0
    end do
    write(15,*) 
    end do
  close(15)
