...
module mprmtr
  ...
  ! controls
  real(sp), parameter :: dt = 2.e-4, dtwrite = 1.5 
...
program rscm
  ...
  real(sp) :: ro, omega, twrite, tmax, time, beta
  real(sp) :: gasdev
  ...
  if (ll .ne. 6) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./rscm.x nini tmax ro beta idum ireal'
  ...
  if ( nini .eq. 0 ) then
    ...
    write(str, '(I20)') nini
    str = adjustl(str)
    str = 'rscm-aij-'//str(1:len_trim(str)) 
    ...
    write(str, '(I20)') nini
    str = adjustl(str)
    str = 'rscm-gi-'//str(1:len_trim(str)) 
    str = str(1:len_trim(str))//'-'//strireal(1:len_trim(strireal))//'.data'
    open(10, file = str(1:len_trim(str)), form = 'binary')
      read(10) gall
    close(10)
  ...
  do while ( time .le. tmax + dt)
    ...
    do ii = 1, npnt
      ...
      call drift(aij, gi, omega, beta, driftaij0, driftgi0)
      aij1 = aij + driftaij0 * dt + dwij
      gi1 = gi + driftgi0 * dt + dwgi

      call drift(aij1, gi1, omega, beta, driftaij1, driftgi1)
...
subroutine drift(aij, gi, omega, beta, driftaij, driftgi)
  use mconstant
  use mprmtr
  implicit none
  ...
  driftaij =  -a2ij - omega*coij - ((2.*o3*omega-tra2 + gi(3)) / trcinvij) * cinvij - (trcinvij / 3) * aij 
