program align3drt
  ...
  complex, allocatable, dimension(:,:,:) :: helidiss
  ...
  real :: delta_c, ignore_me,kcut2, const
  ...
  if (ll .ne. 5) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./align-3drt-cndm.x nx filelist disslist ndel nthresh'
          write(*,*) '        nx: resolution of data'
          write(*,*) '        filelist: the list of the data files, *.list'
          write(*,*) '        disslist: dissipation data file list'
          write(*,*) '        ndel: filter scale Delta=ndel*dx'
          write(*,*) '        nthresh: threshold value = -nthresh * mhelidiss'
          write(*,*)
          write(*,*) ' Stopped'
          stop
  end if
  ...
  str='-'//str(1:len_trim(str))//'dx-rt-cndm'//disslist(1:len_trim(disslist))//'-'//fnm(1:len_trim(fnm))//'.dat'
  ...
  allocate(g(lx1,ly,lz),gcut(lx1,ly,lz),helidiss(lx1,ly,lz))
  ...
  open(21, file = disslist( 1 : len_trim( disslist ) ) )
    ...
    do while ( .not. eof(20) ) 
      ...
      if ( numpnt .eq. 0 ) then
        mhelidiss = sum( real( helidiss(1:lx,:,:) ) ) + sum( aimag( &
                    helidiss(1:lx,:,:) ) ) 
        mhelidiss = mhelidiss / (nx * ny * nz)

        write(*,*) 'estimated helicity dissipation: ', mhelidiss
      end if
      ...
      do ii=1,nx
        ...
        if ( ignore_me .le. -nthresh * mhelidiss ) then
  ...
  deallocate(g,gcut,kx,ky,kz,helidiss)
