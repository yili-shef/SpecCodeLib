program align3drt
  ...
  if (ll .ne. 5) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./align-3drt-cnd.x nx filelist disslist ndel nthresh'
  ...
  str='-'//str(1:len_trim(str))//'dx-rt-cnd'//disslist(1:len_trim(disslist))//'-'//fnm(1:len_trim(fnm))//'.dat'
  ...
  open(21, file = disslist( 1 : len_trim( disslist ) ) )
    ...
    do while ( .not. eof(20) ) 
      ...
      do ii=1,nx
        ...
        if ( ignore_me .ge. nthresh * mhelidiss ) then
