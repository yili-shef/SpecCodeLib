program stratified
  ...
  write(*,*) 'Brunt-Vasaila frequency', bvfreq
  write(*,*) 'iseed', iseed
  ...
  do while (time .le. timemax)
    ...
    if (abs(time-twrite) .le. smallest) then
      ...
      call outputvel2c(va,vb,ifile,lx1,ly,lz)
    ...
    call convecphi_dns(vx,vy,vz,phi,wphi,kx,ky,kz,k2,lx1,ly,lz,nx,ny,nz)
