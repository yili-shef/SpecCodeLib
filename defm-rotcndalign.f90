program defmcijcorbw
  ...
  integer :: nx,ny,nz,ii,jj,kk,lx1,lx,ly,lz,ll,mm,pp,nfile, ifile, filecnt
  ...
  real(dp), dimension(evsize) :: evcc, omp, ominc, rotinc, rotinc0, ominc0, ecross
  real(dp) :: e1, e2, e3

  real(dp) :: align, cntr1, cntr2, costh
...
!      roz(1:nx:2,:,:)=real(omz(1:lx,:,:),sp); roz(2:nx:2,:,:)=aimag(omz(1:lx,:,:))
      ...
      do mm = 1, npntgrp
        ...
        if (mm .eq. 1) then
            ...
            mcntr1(ifile) = mcntr1(ifile) + cntr1 
            mcntr2(ifile) = mcntr2(ifile) + cntr2
