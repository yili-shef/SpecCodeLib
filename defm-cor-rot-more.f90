program defmcijcorbw
  ...
  real(dp), dimension(npdf) :: pcosth, rotcnd1, rotcnd2
  ...
  real(dp), dimension(evsize) :: evcc, evcc0, omp, rotinlab, rotinlab0
  real(dp), dimension(evsize) :: straininc, srot, srot0, omp0, ecross

  real(dp) :: mcntr1, mcntr2, align, cntr1, cntr2, costh
  ...
  if (iargc() .ne. 6) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./defm-cor-rot-more.x nx filelist prefix inicoors npntgrp fw'
  ...
  open(27, file = 'defm-rotcorel-more1-'//trim(str)//'-'//trim(flnm)//'.dat')
  open(28, file = 'defm-rotcorel-more2-'//trim(str)//'-'//trim(flnm)//'.dat')
  ...
  do while ( .not. eof(30) )
    ...
    do mm = 1, npntgrp
      ...
      ! Note defined in ascending order. Will take dot_prod, order not important
      ! largest 
      straininc(3) = dot_product(evtrcc(:,2), matmul(sij, evtrcc(:,1))) &
                        * ( evcc(2) + evcc(1) ) / ( evcc(2) - evcc(1) )
      ! intermediate
      straininc(2) = dot_product(evtrcc(:,1), matmul(sij, evtrcc(:,3))) &
                        * ( evcc(3) + evcc(1) ) / ( evcc(1) - evcc(3) )
      ! smallest 
      straininc(1) = dot_product(evtrcc(:,3), matmul(sij, evtrcc(:,2))) &
                        * ( evcc(3) + evcc(2) ) / ( evcc(3) - evcc(2) ) 

      ! strain rate contribution in Lab frame
      srot(1) = dot_product(straininc, evtrcc(1,:))
      srot(2) = dot_product(straininc, evtrcc(2,:))
      srot(3) = dot_product(straininc, evtrcc(3,:))

      ! rotation rate in lab frame
      rotinlab(1) = omp(1) * .5_dp + srot(1)
      rotinlab(2) = omp(2) * .5_dp + srot(2)
      rotinlab(3) = omp(3) * .5_dp + srot(3)
      ...
      if (mm .eq. 1) then
          ...
          ! in Lab frame
          ecross(1) = evtrcc0(2,3) * evtrcc(3,3) - evtrcc0(3,3) * evtrcc(2,3)
          ecross(2) = evtrcc0(3,3) * evtrcc(1,3) - evtrcc0(1,3) * evtrcc(3,3)
          ecross(3) = evtrcc0(1,3) * evtrcc(2,3) - evtrcc0(2,3) * evtrcc(1,3)

          cntr1 = 0.5_sp * dot_product( omp0 - omp, ecross ) * align
          cntr2 = dot_product( srot0 - srot, ecross ) * align
          ...
          rotcnd1(ii) = rotcnd1(ii) + cntr1
          rotcnd2(ii) = rotcnd2(ii) + cntr2
      end if
    ...
    write(27, '(I6, 25E15.3)') nfile, mcntr1,  mcntr2

    write(28, '("# index = ", I6)') nfile
    do jj = 1, npdf
      write(28, '(15E15.3)')  binw * (jj-.5_sp), pcosth(jj), rotcnd1(jj), rotcnd2(jj)
    end do
    write(28,*)
    write(28,*)
  ...
  write(*,*) 'defm-cor-rot-more.x done.'
