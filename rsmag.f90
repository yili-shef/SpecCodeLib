subroutine rsmag (vx,vy,vz,t11,t12,t13,t22,t23,t33,kx,ky,kz,lx1,ly,lz, &
                 nx,ny,nz,delta)
  use mconstant
  use mfftwplan3d
  implicit none
  ...
  do ii = 1, lx1 - 1
    ...
    t11(ii,jj,kk) = -2. * c1 * delta * delta * t11(ii,jj,kk)
    t12(ii,jj,kk) = -2. * c1 * delta * delta * t12(ii,jj,kk)
    t13(ii,jj,kk) = -2. * c1 * delta * delta * t13(ii,jj,kk)
    t22(ii,jj,kk) = -2. * c1 * delta * delta * t22(ii,jj,kk)
    t23(ii,jj,kk) = -2. * c1 * delta * delta * t23(ii,jj,kk)
    t33(ii,jj,kk) = -2. * c1 * delta * delta * t33(ii,jj,kk)

  end do
  end do
  end do
  
end subroutine rsmag
