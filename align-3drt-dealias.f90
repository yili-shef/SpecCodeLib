program align3drt
  ...
  real(dp), dimension(npnt) :: praltal, praltbe, praltga
  real(dp), dimension(npnt) :: prbetal, prbetbe, prbetga
  real(dp), dimension(npnt) :: prgatal, prgatbe, prgatga
  ...
  if (ll .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./align-3drt-dealias.x nx filelist ndel'
  ...
  str='-'//str(1:len_trim(str))//'dx-rt-dealias-'//fnm(1:len_trim(fnm))//'.dat'
  ...
  praltal = 0._dp; praltbe = 0._dp; praltga = 0._dp
  prbetal = 0._dp; prbetbe = 0._dp; prbetga = 0._dp
  prgatal = 0._dp; prgatbe = 0._dp; prgatga = 0._dp
  pcthphi = 0.0_dp; pcthzeta = 0.0_dp; pphizeta = 0.0_dp
  pcth = 0.0_dp; pphi = 0.0_dp; pzeta = 0.0_dp
  p3d = 0.0_dp
  nfile = 0
  open(20,file=fnm(1:len_trim(fnm)))
    ...
    do while ( .not. eof(20)) 
      ...
      do ii=1,nx
        ...
        cos1   = abs(sum(evrij(:,3)*evtauij(:,3)))
        cos2   = abs(sum(evrij(:,3)*evtauij(:,2)))
        ctheta = abs(sum(evrij(:,3)*evtauij(:,1)))

        ll=floor(cos1/bwcth)+1
        praltal(ll) = praltal(ll) + 1
        ll=floor(cos2/bwcth)+1
        praltbe(ll) = praltbe(ll) + 1
        ll=floor(ctheta/bwcth)+1
        praltga(ll) = praltga(ll) + 1

        cos1   = abs(sum(evrij(:,2)*evtauij(:,3)))
        cos2   = abs(sum(evrij(:,2)*evtauij(:,2)))
        ctheta = abs(sum(evrij(:,2)*evtauij(:,1)))

        ll=floor(cos1/bwcth)+1
        prbetal(ll) = prbetal(ll) + 1
        ll=floor(cos2/bwcth)+1
        prbetbe(ll) = prbetbe(ll) + 1
        ll=floor(ctheta/bwcth)+1
        prbetga(ll) = prbetga(ll) + 1

        cos1   = abs(sum(evrij(:,1)*evtauij(:,3)))
        cos2   = abs(sum(evrij(:,1)*evtauij(:,2)))
        ctheta = abs(sum(evrij(:,1)*evtauij(:,1)))

        ll=floor(cos1/bwcth)+1
        prgatal(ll) = prgatal(ll) + 1
        ll=floor(cos2/bwcth)+1
        prgatbe(ll) = prgatbe(ll) + 1
        ll=floor(ctheta/bwcth)+1
        prgatga(ll) = prgatga(ll) + 1
  ...
  praltal = praltal * const / nfile / bwcth
  praltbe = praltbe * const / nfile / bwcth
  praltga = praltga * const / nfile / bwcth

  prbetal = prbetal * const / nfile / bwcth
  prbetbe = prbetbe * const / nfile / bwcth
  prbetga = prbetga * const / nfile / bwcth

  prgatal = prgatal * const / nfile / bwcth
  prgatbe = prgatbe * const / nfile / bwcth
  prgatga = prgatga * const / nfile / bwcth

  open(15,file='jpalign-ralphatau'//str(1:len_trim(str)))
    do ii=1,npnt
      write(15,'(15E15.5)') (ii-.5)*bwcth, praltal(ii), praltbe(ii), praltga(ii)
    end do
  close(15)
  open(15,file='jpalign-rbetatau'//str(1:len_trim(str)))
    do ii=1,npnt
      write(15,'(15E15.5)') (ii-.5)*bwcth, prbetal(ii), prbetbe(ii), prbetga(ii)
    end do
  close(15)
  open(15,file='jpalign-rgammatau'//str(1:len_trim(str)))
    do ii=1,npnt
      write(15,'(15E15.5)') (ii-.5)*bwcth, prgatal(ii), prgatbe(ii), prgatga(ii)
    end do
  close(15)
  ...
  write(*,*) 'align-3drt-dealias.x finished'
