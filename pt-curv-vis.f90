program ptcurvvis
  ...
  real(sp) :: meantvb, meanxigradvn, meantor, torsionp, c2
  ...
  real(sp),    allocatable, dimension(:,:,:) :: g, torsion, k2
  ...
  if (nx .ne. 6) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./pt-curv-vis.x nx filelist ndel nprtcle dt prefix'
  ...
  allocate( torsion(nx,ny,nz), k2(lx1,ly,lz) )
  ...
  open(25, file = 'curv-vis-'//flnm(1:len_trim(flnm))//'.dat')
  ...
  do while ( .not. eof(30) )
    ...
    do ii = 1, lx1
        ...
        ! omega_i
        g11(ii,jj,kk) = eye * ( ky(jj) * g32(ii,jj,kk) - kz(kk) * g31(ii,jj,kk) )
        g22(ii,jj,kk) = eye * ( kz(kk) * g21(ii,jj,kk) - kx(ii) * g32(ii,jj,kk) )
        g33(ii,jj,kk) = eye * ( kx(ii) * g31(ii,jj,kk) - ky(jj) * g21(ii,jj,kk) )

        ! v_i = rnu * grad^2 omega_i
        g11(ii,jj,kk) = -k2(ii,jj,kk) * rnu * g11(ii,jj,kk)
        g22(ii,jj,kk) = -k2(ii,jj,kk) * rnu * g22(ii,jj,kk)
        g33(ii,jj,kk) = -k2(ii,jj,kk) * rnu * g33(ii,jj,kk)
    ...
    do ii = 1, nx
        if ( mod(ii,2) .eq. 1 ) then
            ...
            ! g21 = rnu * grad^2 omega_i dot c_i
            ! g31 = c * omega
            ! g32 = 1 / (c * omega)
    ...
    do ip=1, nprtcle
      ...
      meantvb = meantvb + torsionp

    end do
    meantvb =  meantvb / nprtcle 
    ...
    do ip=1, nprtcle
      ...
      meanxigradvn = meanxigradvn + torsionp

    end do
    meanxigradvn =  meanxigradvn / nprtcle 


    write(25, '(10E15.4)') time, meantor, -meantvb, meanxigradvn
    time = time + dt
