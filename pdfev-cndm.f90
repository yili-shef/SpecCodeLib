program pdfevcndm
  ...
  integer :: nx,ny,nz,ii,jj,kk,ll,mm,nn,nthresh,lx,lx1,ly,lz,nfile
  integer(8) :: numpnt
  real, allocatable, dimension(:,:,:) :: alpha, beta, gmma
  complex, allocatable, dimension(:,:,:) :: dissrate

  integer, parameter :: npnt=100
  real,    parameter :: bnd=10., binw1=2./npnt, binw2=bnd/npnt
  ...
  real(dp) :: sbeta, mdissrate
  real :: rmsr0, atmp, btmp, gtmp, ss
  real(dp) :: const
  ...
  if (ll .ne. 4) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./pdfev-cndm.x nx filelist disslist nthresh'
          write(*,*) '        nx: resolution of data'
          write(*,*) '        filelist: the list of the eigenvalue data files'
  ...
  str='-'//fnm(1:len_trim(fnm))//'-cndhdm'//disslist(1:len_trim(disslist))//'.dat'
  ...
  open(21, file = disslist( 1 : len_trim( disslist ) ) )
    ...
    malpha = 0.d0; mbeta = 0.d0; mgamma = 0.d0
    ralpha = 0.d0; rbeta = 0.d0; rgamma = 0.d0
    rmsr=0.d0
    sbeta = 0.d0 
    psat=0.d0
    pal=0.d0; pbe=0.d0; pga=0.d0
    ...
    do while ( .not. eof(20)) 
      ...
      if (nfile .eq. 0) then
        ...
        write(*,*) 'Estimate of mdissrate = ', mdissrate
      end if
      ...
      do ii=1,nx
        if ( mod(ii, 2) .eq. 1) then
              const = real( dissrate( (ii+1)/2, jj, kk ) )
        else 
              const = aimag( dissrate( ii/2, jj,kk ) )
        end if
        ...
        if ( const .le. - nthresh * mdissrate ) then
          numpnt = numpnt + 1
