program cndmeanxo
  ...
  write(*,*) 
  write(*,'(''>>>>>> Conditional averaging between omega and xi <<<<<<'')')
  write(*,*)
  ll=iargc()
  if (ll .ne. 3) then
          write(*,*)
          ...
          write(*,*) 
          write(*,*) ' Usage: ./cndmean-xo-s(d)p.x nx filelist ndel'
    ...
    do while ( .not. eof(20)) 
      ...
      do ii = 1, nx
        ...
        if ( ll .ge. 1 .and. ll .le. npnto .and. mm .ge. 1 .and. &
          ...
          pdfo(ll) = pdfo(ll) + 1
          pdfx(mm) = pdfx(mm) + 1
          xcndo(ll) = xcndo(ll) + nxnorm
          ocndx(mm) = ocndx(mm) + nwnorm
        end if
  ...
  open(15, file ='cndpdfxo-'//str(1:len_trim(str))//'dx-'//fnm(1:len_trim(fnm))//'.dat' )
    write(15,*) '# Zone T= "(omega,xi)", i=', npnto, ', j=', npntx, ', F=point'
    do ii = 1, npnto
    do jj = 1, npntx
      write(15,'(15E15.4)') (ii-.5)*bwo*rmso0/rmso, (jj-.5)*bwx*rmsx0/rmsx, &
                  jox(ii,jj)*rmso*rmsx/rmso0/rmsx0/(pdfo(ii)*rmso/rmso0+mytiny_dp), &
                  jox(ii,jj)*rmso*rmsx/rmso0/rmsx0/(pdfx(jj)*rmsx/rmsx0+mytiny_dp)
    end do
    write(15,*)
    end do
  close(15)
