program langmodpdfavr
  ...
  real(dp) :: meana120, rmsa120

  integer :: ii, jj, ll, npnt
  real(sp) :: tmp
  character(80) :: str, strro

  write(*,*)
  write(*,'('' >>> Lagrangian model for velocity gradient <<< '')')
  write(*,*) 
  ll = iarg()
  if (ll .ne. 2) then
          write(*,*)
          ...
          write(*,*) 
          write(*,*) ' Usage: ./rcm-pdf-avr-s(d)p.x filelist npnt'
          write(*,*) '        filelist: filelist.list is the list of data files'
          write(*,*) '        npnt: number of realizations in each data'
          write(*,*)
          write(*,*)
          write(*,*) ' Stopped'
          stop 
  ...
  str = 'pdfaiiavr-'//strro(1:len_trim(strro))//'.dat'
  ...
  str = 'pdfaijavr-'//strro(1:len_trim(strro))//'.dat'
