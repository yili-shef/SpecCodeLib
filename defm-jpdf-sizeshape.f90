program defmjpdfsizeshape
  ...
  real(dp), dimension(npnt,npntsz) :: jpdfss
  real(dp), dimension(npnt) :: pdfss
  real(dp), dimension(npntsz) :: pdfsz
  ...
  if (iargc() .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./defm-jpdf-sizeshape.x nx filelist prefix'
  ...
  open(27, file = 'jpdf-size-shape-'//trim(flnm)//'.dat')
  write(27, '("# size shape  jpdf   ")') 
  write(27, *)
  ...
  do while ( .not. eof(30) )
    ...
    pdfss  = pdfss  * const
    pdfsz  = pdfsz  * const
    write(*,*) 'normalization of jpdfss: ', sum(jpdfss)
    jpdfss = jpdfss / binw / binwsz
    pdfss  = pdfss  / binw
    pdfsz  = pdfsz  / binwsz
    ...
    do ll = 1, npnt
    do mm = 1, npntsz
      write(27, '(15E15.3)') -1 + (ll-0.5_sp)*binw, (mm-0.5_sp)*binwsz, jpdfss(ll,mm), &
                             pdfss(ll) * pdfsz(mm)
    end do
    write(27, *) 
    end do
  ...
  write(*,*) 'defm-jpdf-sizeshape.x done.'

end program defmjpdfsizeshape
