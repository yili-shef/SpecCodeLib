program velincmoments
  ...
  if (nx .ne. 2) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./vel-inc-moments.x nx filelist'
  ...
  allocate( uxr(nx, ny, nz), uyr(nx, ny, nz), dur(nx, ny, nz), tmp(nx, ny, nz) )
  ...
  do while ( .not. eof(30) )
    ...
    call rfftwnd_f77_one_complex_to_real(c3r3d,ux,ignore_me)
    call rfftwnd_f77_one_complex_to_real(c3r3d,uy,ignore_me)
  ...
  path='vel-inc-moments-'//flnm(1:len_trim(flnm))//'.dat'//char(0)
