program visualox
  ...
  write(*,*) 
  write(*,'(''>>>>>> Visualization the relation between magnitude of xi and vortex lines <<<<<<'')')
  write(*,*)
  ll=iargc()
  if (ll .ne. 3) then
          write(*,*)
          ...
          write(*,*) 
          write(*,*) ' Usage: ./visual-xo.x nx filenum ndel'
          write(*,*) '        nx: resolution of data'
          write(*,*) '        filenum: the number of dns data file'
  ...
  open(15, file='poxalign-1d'//fnm(1:len_trim(fnm)))
    do ii=1,npnt
      write(15,'(15E15.6)') (ii-.5)*binw, palpha(ii),pbeta(ii),pgamma(ii)
    end do
  close(15)

  open(15, file = 'poxalign-2d'//fnm(1 : len_trim( fnm )))
