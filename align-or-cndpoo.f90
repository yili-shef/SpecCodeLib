program alignor
  ...
  complex(sp), allocatable, dimension(:,:,:) :: wx,wy,wz, oo
  ...
  real(sp) :: delta_c, ignore_me, tmp, tmp1, rmso0, kcut2
  real(dp) :: alpha, beta, gamm, rmso
  ...
  if (ll .ne. 4) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./align-or-cndpoo.x nx filelist ndel nthresh'
          write(*,*) '        nx: resolution of data'
          write(*,*) '        filelist: dns data file list'
          write(*,*) '        ndel: filter scale delta=ndel*dx'
          write(*,*) '        nthresh: threshold vorticity = nthresh * rms vorticity'
          write(*,*)
          write(*,*) ' Stopped'
          stop
  end if
  ...
  str = str(1:len_trim(str))//'dx-cndpoo'//fpath(1:len_trim(fpath))
  ...
  allocate(kx(lx1),ky(ly),kz(lz), oo(lx1,ly,lz) )
  ...
  do while ( .not. eof(20) )
    ...
    do ii = 1, nx
      ...
      if ( mod(ii,2) .eq. 1 ) then 
        ll = (ii + 1)/2
        tmp1 = sqrt( real( oo(ll,jj,kk) ) )
      else
        ll = ii / 2
        tmp1 = sqrt( aimag( oo(ll,jj,kk) ) )
      end if

      if ( tmp1 .ge. nthresh * rmso0 ) then
  ...
  write(*,*) 'rms of vorticity: ', rmso

  rmso = 1._dp/numpnt

  write(*,*) 'check palpha:', sum(palpha)*rmso
  write(*,*) 'check pbeta:',  sum(pbeta)*rmso
  write(*,*) 'check pgamma:', sum(pgamma)*rmso
  write(*,*) 'check p2d:   ', sum(p2d)*rmso

  palpha=palpha*rmso/binw
  pbeta=pbeta*rmso/binw
  pgamma=pgamma*rmso/binw

  p2d = p2d * rmso / ( binw * binwphi )
  ...
  deallocate(wx,wy,wz,g,kx,ky,kz, oo, gcut)
