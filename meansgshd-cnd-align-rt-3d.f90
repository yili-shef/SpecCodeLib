...
!  real(sp), parameter :: mdissrate0 = 0.2375 ! Delta =  8 dx
  ...
  real(sp) :: delta_c, ignore_me, pih, kcut2
  ...
  if (ll .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./meansgshd-cnd-align-rt-3d.x nx filelist ndel'
  ...
  cndhd3d = cndhd3d / (jpdf3d + mytiny) 
  cndhdcthphi = cndhdcthphi / (jpdfcthphi + mytiny)
  cndhdcthzeta = cndhdcthzeta / (jpdfcthzeta + mytiny)
  cndhdphizeta = cndhdphizeta / (jpdfphizeta + mytiny)
  ...
  open(15,file='cndsgshd-align-rt-3d'//str(1:len_trim(str)))
  ...
  open(15,file='cndsgshd-align-rt-2d'//str(1:len_trim(str)))
