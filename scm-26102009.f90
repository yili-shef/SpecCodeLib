...
program stratifiedcm
  ...
  real(sp) :: twrite, tmax, time, nbv, tmp
  real(sp) :: gasdev
  character(80) :: str, strireal, strnbv
  ...
  call getarg(2,str)
  read(str, '(F10.6)') tmax
  call getarg(3,strnbv)
  read(strnbv, '(F10.6)') nbv
  call getarg(4, str)
  read(str, '(I20)') idum
  ...
  if ( nini .eq. 0 ) then
    ...
    write(str, '(I20)') nini
    str = adjustl(str)
    str = 'cm-aij-nbv-'//strnbv(1:len_trim(strnbv))//'-'//str(1:len_trim(str)) 
    ...
    write(str, '(I20)') nini
    str = adjustl(str)
    str = 'cm-gi-nbv-'//strnbv(1:len_trim(strnbv))//'-'//str(1:len_trim(str)) 
    str = str(1:len_trim(str))//'-'//strireal(1:len_trim(strireal))//'.data'
    open(10, file = str(1:len_trim(str)), form = 'binary')
      read(10) gall
    close(10)
  ...
  do while ( time .le. tmax + dt)
    ...
    do ii = 1, npnt
      ...
      call drift(aij, gi, nbv, driftaij0, driftgi0)
      aij1 = aij + driftaij0 * dt + dwij
      gi1 = gi + driftgi0 * dt + dwgi

      call drift(aij1, gi1, nbv, driftaij1, driftgi1)
...
end program stratifiedcm      

subroutine drift(aij, gi, nbv, driftaij, driftgi)
  use mconstant
  use mprmtr
  implicit none
  ...
  driftgi = - nbv*nbv * aij(3,:) - matmul(transpose(aij), gi) - trcinvij / (3. * timetheta) * gi
