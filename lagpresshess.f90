program lagph
  ...
  real(sp) :: meanFbnm, Fbnm, meanafn, afntmp, akk, meanakk, trc
  real(dp) :: alpha, beta, gmma, meanal, meanbe, meangm
  
  if (iargc() .ne. 3) then
          write(*,*)
          write(*,*) ' >>>>>> Wrong number of arguments <<<<<<'
          write(*,*) 
          write(*,*) ' Usage: ./lagpresshess.x nx filelist prefix'
  ...
  open(27, file = 'aniso-'//trim(flnm)//'.dat')
  write(27,*) '# nfile meanal meanbe meangm trace(pij) trace(cij) '
  open(28, file = 'anisoCndAfn-'//trim(flnm)//'.dat')
  ...
  do while ( .not. eof(30) )
    ...
    do ii = 1, nx
      ...
      alpha = evalues(3); beta = evalues(2); gmma = evalues(1)

      meanal = meanal + alpha
      meanbe = meanbe + beta
      meangm = meangm + gmma

      alpha = alpha / trc + evbnd
    ...
    meanbe = meanbe * const
    meangm = meangm * const
    write(27, '(I5, 15E15.3)') nfile, meanal, meanbe, meangm, meanakk, trc
