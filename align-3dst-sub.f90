program align3dst
  ...
  write(*,*) 
  write(*,'(''>>> Joint PDFs of the alignments between sij and sgs stress minor Leonard stress <<<'')')
  write(*,*)
  ll=iargc()
  if (ll .ne. 3) then
          write(*,*)
          ...
          write(*,*) 
          write(*,*) ' Usage: ./align-3dst-sub.x nx filelist ndel'
  ...
  str='-'//str(1:len_trim(str))//'dx-stsub-'//fnm(1:len_trim(fnm))//'.dat'
  ...
  open(20,file=fnm(1:len_trim(fnm)))
    ...
    do while ( .not. eof(20)) 
      ...
      ! Leonard stress
      call rsgstauij(ux,ux,s11,g,nx,ny,nz) 
      call rsgstauij(ux,uy,s12,g,nx,ny,nz)
      call rsgstauij(ux,uz,s13,g,nx,ny,nz)
      call rsgstauij(uy,uy,s22,g,nx,ny,nz)
      call rsgstauij(uy,uz,s23,g,nx,ny,nz)
      call rsgstauij(uz,uz,s33,g,nx,ny,nz)
      ...
      do jj = 1, ly
      do ii = 1, lx1
        s12(ii,jj,kk)=.5*eye*(kx(ii)*uy(ii,jj,kk)+ky(jj)*ux(ii,jj,kk))
        s13(ii,jj,kk)=.5*eye*(kx(ii)*uz(ii,jj,kk)+kz(kk)*ux(ii,jj,kk))
        s23(ii,jj,kk)=.5*eye*(ky(jj)*uz(ii,jj,kk)+kz(kk)*uy(ii,jj,kk))
        s11(ii,jj,kk)=eye*kx(ii)*ux(ii,jj,kk)
        s22(ii,jj,kk)=eye*ky(jj)*uy(ii,jj,kk)
        s33(ii,jj,kk)=eye*kz(kk)*uz(ii,jj,kk)
      end do
      end do
      end do
  ...
  open(15,file='jpalign-1d'//str(1:len_trim(str)))
    do ii=1,npnt
      write(15,'(15E18.4)') (ii-.5)*bwcth, ptha(ii), pthb(ii), pthg(ii)
    end do
  close(15)
  ...
  open(15, file = 'jpalign-3d'//str(1:len_trim(str)))
    ...
    do jj=1,npnt
    do ii=1,npnt
      write(15,'(15E18.4)') (ii-.5) * bwcth, (jj-.5) * bwphi, (kk-.5) * bwzeta, &
      p3d(ii,jj,kk)
    end do
    end do
    end do
  ...
  open(15,file='jpalign-2d'//str(1:len_trim(str)))
    ...
    do jj=1,npnt
    do ii=1,npnt
      write(15,'(15E18.4)') (ii-.5)*bwcth, (jj-.5)*bwphi, pcthphi(ii,jj)
    end do
    end do
    ...
    do jj=1,npnt
    do ii=1,npnt
      write(15,'(15E18.4)') (ii-.5)*bwcth, (jj-.5)*bwzeta, pcthzeta(ii,jj)
    end do
    end do
    ...
    do jj=1,npnt
    do ii=1,npnt
      write(15,'(15E18.4)') (ii-.5)*bwphi, (jj-.5)*bwzeta, pphizeta(ii,jj)
    end do
    end do
  close(15)
  ...
  deallocate(g,k2, kx,ky,kz, ux, uy, uz)
