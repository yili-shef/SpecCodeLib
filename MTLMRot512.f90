...
!========  parameter for velocity field   =========
  ...
  real(8), parameter :: mu = 2. ! exponent of energy spectrum in the inertial range. 
  real(8), parameter :: pexp = 2. ! exponent of energy spec in the production range.
  !real(8), parameter :: omega = urms / (2*l) / ro
...
!....... definitions in fftw3.f ......................................
  ...
  complex(8), dimension(128 + 1,256,256) :: vx256,vy256,vz256 
  complex(8), dimension( 64 + 1,128,128) :: vx128,vy128,vz128 
  complex(8), dimension( 32 + 1, 64, 64) :: vx64 ,vy64 ,vz64
  complex(8), dimension( 16 + 1, 32, 32) :: vx32 ,vy32 ,vz32
  complex(8), dimension(  8 + 1, 16, 16) :: vx16 ,vy16 ,vz16
  complex(8), dimension(  4 + 1,  8,  8) :: vx08 ,vy08 ,vz08
  ...
  if ( iarg() .ne. 3 ) stop "need three arguments: ./MTLMRot.x ro -idum nfile"
  ...
  call getarg(3,cmd) ! cmd is the output file number.
  cmd = adjustl(cmd)
  ...
  omega = urms0 / (2 * l) / ro
  write(*,*) 'omega, urms0, l, ro :', omega, urms0, l, ro
  write(911,*) 'omega, urms0, l, ro :', omega, urms0, l, ro
...
!  close(unit=21)
  ...
  call apply_MTLM(vx08,vy08,vz08,8 + 2,                         &
                  IFTvx08,IFTvy08,IFTvz08,FTvx08,FTvy08,FTvz08)

  call apply_MTLM(vx16,vy16,vz16, 16 + 2,                         &
                  IFTvx16,IFTvy16,IFTvz16,FTvx16,FTvy16,FTvz16)

  call apply_MTLM(vx32,vy32,vz32, 32 + 2,                          &
                  IFTvx32,IFTvy32,IFTvz32,FTvx32,FTvy32,FTvz32)

  call apply_MTLM(vx64,vy64,vz64, 64 + 2,                          &
                  IFTvx64,IFTvy64,IFTvz64,FTvx64,FTvy64,FTvz64)
 
  call apply_MTLM(vx128,vy128,vz128, 128+ 2,IFTvx128,IFTvy128,    &
                  IFTvz128,FTvx128,FTvy128,FTvz128)
  
  call apply_MTLM(vx256,vy256,vz256, 256 + 2,IFTvx256,IFTvy256,    &
...
!****** 3-D spectrum function ********
    ...
    if ( .not. fromfile ) then
        ...
        !if ( kl .ge. fkint ) then 
        !    spec3d = urms**2*l *(kl/fkint)**(-mu)
        !else
        !    spec3d = urms**2*l *(kl/fkint)**pexp
        !end if
        spec3d = urms**2*l *kl**(-mu) &
               * (kl/sqrt(kl*kl + cl))**(mu + p0)    &
               * dexp(-1.5d0*Ck*(rk*eta)**4.d0/3.d0)
...
SUBROUTINE MTLM(vxred,vyred,vzred,ldr,tadv,urmsb)
  ...
  !****(1) 
     ...
     ir = dble(ia1) + va1 * ( 1 - cos(2*omega*dt) ) * vnm + ua1 * sin(2*omega*dt) * vnm
     jr = dble(ja) + va1 * sin(2*omega*dt) * vnm + ua1 * (cos(2*omega*dt) - 1) * vnm
     kr = dble(ka) + dt * wa1 / hxr

     ua2 = ua1 *cos(2*omega*dt) + va1 * sin(2*omega*dt)
     va1 = va1 *cos(2*omega*dt) - ua1 * sin(2*omega*dt)
     ua1 = ua2
  ...
  !****(2) 
     ...
     ir = dble(ia2) + va2 * ( 1 - cos(2*omega*dt) ) * vnm + ua2 * sin(2*omega*dt) * vnm
     jr = dble(ja) + va2 * sin(2*omega*dt) * vnm + ua2 * (cos(2*omega*dt) - 1) * vnm
     kr = dble(ka) + dt * wa2 / hxr

     ua1 = ua2 *cos(2*omega*dt) + va2 * sin(2*omega*dt)
     va2 = va2 *cos(2*omega*dt) - ua2 * sin(2*omega*dt)
     ua2 = ua1
  ...
  DO ! while (voids > 0) ! By YL
    ...
    do i = 1,nxr
      ...
      do isearch = is(i),ie(i)
         ...
         ip = dble(in) + va1 * ( 1 - cos(2*omega*dt) ) * vnm + ua1 * sin(2*omega*dt) * vnm
         jp = dble(jn) + va1 * sin(2*omega*dt) * vnm + ua1 * (cos(2*omega*dt) - 1) * vnm
         kp = dble(kn) + dt * wa1 / hxr
         ua2 = ua1 *cos(2*omega*dt) + va1 * sin(2*omega*dt)
         va1 = va1 *cos(2*omega*dt) - ua1 * sin(2*omega*dt)
         ua1 = ua2
...
function caltimescale(delta) result(timescale)
  ...
  real(8) :: timescale
  real(8), parameter :: alpha = 0.44

  real(8), parameter :: epsi = 0.1
  ...
  timescale = min(1. / sqrt(tau), 1./(2*omega)) ! For rotating turbulence.
  !timescale = 1 / sqrt(tau) ! For rotating turbulence: another possibility

  !timescale = (delta * delta / epsi)**(1./3.) ! For FromFileLowRoLimit
