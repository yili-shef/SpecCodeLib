program sijlagpijcndss
  ...
  real(sp), dimension(npnt) :: pdfss, spcndss
  ...
  if (iargc() .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./sijlagpijcndsijsij.x nx filelist prefix'
  ...
  open(28, file = 'sijpijcndsijsij-'//trim(flnm)//'.dat')
  ...
  do while ( .not. eof(30) )
    ...
    call rfftwnd_f77_one_complex_to_real(c2r3d,p12,ignore_me)
    afn = afn + 2._sp * cmplx( real(p12,sp) * real(p12,sp), aimag(p12) * aimag(p12) )
    ...
    ! afn is the Frobenious norm of sij: sijsij
    ! p13 is the correlation sijpij
    ...
    do ii = 1, nx
      ...
      if ( ll .ge. 1 .and. ll .le. npnt ) then 
          pdfss(ll) = pdfss(ll) + 1
          spcndss(ll) = spcndss(ll) + aptmp/trc
      end if

    end do
    end do
    end do
    spcndss = spcndss / (pdfss + mytiny) 
    pdfss = pdfss * const 
    write(*,*) 'normalization of pdfss', sum(pdfss)
    pdfss = pdfss / binw

    write(28, '("# index = ", I6)') nfile
    do ll = 1, npnt
      write(28, '(15E15.3)') (ll - 0.5_sp) * binw, spcndss(ll), pdfss(ll)
    end do
    write(28, *)
    write(28, *)
  ...
  write(*,*) 'sijlagpijcndsijsij.x done.'

end program sijlagpijcndss
