program spectral_dns
  ...
  real(sp) :: timemax, dt, rnu, dtw, time, eps, fkmax, akp, u0
  logical  :: new
  ...
  real(sp) :: ek, dt_h, twrite, fkmax2, kcut2
  real(sp) :: coeforce, kinet, s

  integer, save :: TID

  !$omp threadprivate(TID)

  open(90,file='parameter_forceddnsomp_ft.d',status='unknown')
    read(90,*) nx
    read(90,*) ny
    read(90,*) nz
    read(90,*) timemax
  ...
  do while (time .le. timemax)
    ...
    where (k2 .lt. fkmax2)
      tmp = real( vx*conjg(vx) + vy*conjg(vy) + vz*conjg(vz) )
    elsewhere
      tmp = 0.
    end where
