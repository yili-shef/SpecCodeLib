program defmkinshape
  use mconstant
  use mwavenumber
  use mfftwplan3d
  implicit none
  
  integer :: nx,ny,nz,ii,jj,kk,lx1,lx,ly,lz,ll,mm,nfile, pointcnt
  real(sp) :: ignore_me, const

  integer,  parameter :: npnt = 20
  real(sp), parameter :: bnd = 1._sp, binw = 2._sp* bnd/npnt

  real(dp), dimension(npnt) :: pdfsstar, cndkin, cndmom
  ...
  real(dp) :: kin, ix, iy, iz, angularmom, meanaijaij, meantrcc, meankin, meanmom
  real(dp) :: sstar
  real(sp) :: detbij
  ...
  if (iargc() .ne. 3) then
          ...
          write(*,*) 
          write(*,*) ' Usage: ./defm-kin-shape.x nx filelist prefix'
  ...
  open(27, file = 'kincndshape-'//trim(flnm)//'.dat')
  write(27, '("# sstar  pdfsstar  cndkin   cndmom")') 
  ...
  do while ( .not. eof(30) )
    ...
    meankin = sum(cndkin) * const
    meanmom = sum(cndmom) * const

    cndkin = cndkin / (pdfsstar + mytiny)
    cndmom = cndmom / (pdfsstar + mytiny)
    pdfsstar = pdfsstar * const / binw

    cndkin = cndkin / meankin
    cndmom = cndmom / meanmom
    ...
    do ll = 1, npnt
      write(27, '(15E15.3)') -1 + (ll - 0.5_sp) * binw, pdfsstar(ll), cndkin(ll), cndmom(ll)
    end do
    write(27, *)
    write(27, *)
  ...
  write(*,*) 'defmkinshape.x done.'

end program defmkinshape
