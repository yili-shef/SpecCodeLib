program jpdfpihsstar
  ...
  real(dp), dimension(npnts, npnth) :: jpihsstar
  ...
  if (ii .ne. 4) then
          ...
          write(*,*)
          write(*,*) ' Usage: ./jpdf-pih-sstar.x nx datalist ndel normhelidiss'
    ...
    do while ( .not. eof(20) )
      ...
      do ii = 1, nx
        ...
        if ( ll .ge. 1 .and. ll .le. npnts .and. mm .ge. 1 .and. &
             mm .le. npnth ) then
          jpihsstar(ll,mm) = jpihsstar(ll,mm) + 1
        end if
  ...
  jpihsstar = jpihsstar / nfile * const

  write(*,*) 'Check normalization of jpihsstar: ', sum(jpihsstar) 

  jpihsstar = jpihsstar / bws / bwh

  open(15, file ='jpihsstar'//str(1:len_trim(str)) )
    write(15,*) 'Zone T= "(sstar,pih)", i=', npnts, ', j=', npnth, ', F=point'
    do jj = 1, npnth
    do ii = 1, npnts
      write(15,*) -bounds+(ii-.5)*bws, -boundh+(jj-.5)*bwh, jpihsstar(ii,jj)
    end do
    end do
  close(15)
